...
const calcs = [
  {
    ...
    inputs: [
      { id: 'pricePerUnit', label: 'Verkaufspreis netto pro Stück', type: 'number', defaultValue: 50, min: 0.01, max: 1000000, step: 1, unit: '€' },
      { id: 'variableCostPerUnit', label: 'Variable Stückkosten (Material, Fertigung)', type: 'number', defaultValue: 20, min: 0, max: 1000000, step: 1, unit: '€' },
      { id: 'unitsSold', label: 'Absatzmenge (Verkaufte Stückzahl)', type: 'number', defaultValue: 1000, min: 1, max: 10000000, step: 10, unit: 'Stück' },
      { id: 'fixedCostsTotal', label: 'Gesamte fixe Kosten (Miete, Gehälter etc.)', type: 'number', defaultValue: 20000, min: 0, max: 100000000, step: 500, unit: '€' },
    ],
    calculateCode: `const p = Number(inputs.pricePerUnit) || 0;
...
return {
  ...
  summaryText: 'Bei ' + qty + ' Stück erzielen Sie einen Gesamt-Deckungsbeitrag von ' + formatCurrency(totalDb) + ' (DB-Quote ' + formatPercent(dbRatio) + '). Nach Abzug der Fixkosten von ' + formatCurrency(kf) + ' verbleibt ein Betriebsgewinn von ' + formatCurrency(profit) + '.',
    ...
    workedExample: {
      ...
      inputValues: [{ label: 'Preis', value: '50 €' }, { label: 'Var. Kosten', value: '20 €' }, { label: 'Menge', value: '1.000' }, { label: 'Fixkosten', value: '20.000 €' }],
      steps: ['db = 50 - 20 = 30 € pro Stück', 'DB_ges = 1.000 × 30 € = 30.000 €', 'Gewinn = 30.000 € - 20.000 € = 10.000 €', 'Break-Even = 20.000 / 30 = 667 Stück'],
      result: 'DB = 30.000 €, Gewinn = 10.000 €',
    },
    faqs: [
      { question: 'Was ist der Unterschied zwischen Deckungsbeitrag I und II?', answer: 'Der DB I zieht nur die variablen Kosten vom Umsatz ab. Der mehrstufige DB II zieht zusätzlich die erzeugnisfixenen Kosten ab, um die Rentabilität einzelner Produktlinien präziser zu beurteilen.' },
      { question: 'Was bedeutet eine negative Deckungsbeitragsquote?', answer: 'Ein negativer Deckungsbeitrag bedeutet, dass der Verkaufspreis nicht einmal die unmittelbaren Herstellungskosten deckt. Mit jedem verkauften Stück vergrößert sich der Verlust – das Produkt sollte sofort ausgelistet oder im Preis erhöht werden.' },
    ],
...

  {
    id: 'roas-rechner',
    slug: 'roas-rechner',
    name: 'ROAS Rechner (Return on Ad Spend & Break-Even-ROAS Marketing)',
    ...
    inputs: [
      { id: 'adRevenue', label: 'Durch Werbung erzielter Umsatz', type: 'number', defaultValue: 10000, min: 1, max: 100000000, step: 100, unit: '€' },
      { id: 'adSpend', label: 'Werbeausgaben (Ad Spend)', type: 'number', defaultValue: 2500, min: 1, max: 10000000, step: 50, unit: '€' },
      { id: 'productMarginPct', label: 'Produktmarge / Handelsspanne vor Werbekosten', type: 'number', defaultValue: 50, min: 1, max: 100, step: 1, unit: '%' },
    ],
    calculateCode: `const rev = Number(inputs.adRevenue) || 0;
...
return {
  ...
  summaryText: 'Ihr ROAS beträgt ' + formatNumber(roasFactor, 2) + 'x (' + Math.round(roasPercent) + ' %). Jeder investierte Euro Werbung generiert ' + formatCurrency(roasFactor) + ' Umsatz. Nach Abzug aller Produkt- und Werbekosten verbleibt ein Gewinn von ' + formatCurrency(netProfitFromAds) + '.',
    ...
    workedExample: {
      ...
      inputValues: [{ label: 'Umsatz', value: '10.000 €' }, { label: 'Ad Spend', value: '2.500 €' }, { label: 'Marge', value: '50 %' }],
      steps: ['ROAS = 10.000 / 2.500 = 4,0x (400 %)', 'Break-Even-ROAS = 1 / 0,50 = 2,0x', 'Bruttogewinn = 10.000 × 0,50 = 5.000 €', 'Nettogewinn = 5.000 € - 2.500 € = 2.500 €'],
      result: 'ROAS 4,0x (2.500 € Nettogewinn)',
    },
    faqs: [
      { question: 'Was ist der Unterschied zwischen ROAS und ROI?', answer: 'Der ROAS berücksichtigt ausschließlich die Werbeausgaben (Umsatz / Werbekosten). Der ROI (Return on Investment) berücksichtigt alle Kosten (Wareneinsatz, Personal, Versand, Retouren und Werbung).' },
      { question: 'Welcher ROAS gilt im E-Commerce als gut?', answer: 'Das hängt von der Marge ab: Bei 30 % Marge braucht man mindestens 3,33x ROAS für den Break-Even. Bei 70 % Marge reicht bereits 1,43x ROAS für die Gewinnzone.' },
    ],
...

  {
    ...
    inputs: [
      { id: 'investmentCost', label: 'Einmalige Investitionskosten', type: 'number', defaultValue: 50000, min: 100, max: 100000000, step: 500, unit: '€' },
      { id: 'annualNetGain', label: 'Jährlicher Netto-Gewinnzuwachs / Ersparnis', type: 'number', defaultValue: 15000, min: 1, max: 10000000, step: 250, unit: '€/Jahr' },
      { id: 'holdingYears', label: 'Betrachtungszeitraum der Investition', type: 'number', defaultValue: 5, min: 1, max: 30, step: 1, unit: 'Jahre' },
    ],
    calculateCode: `const inv = Number(inputs.investmentCost) || 1;
...
return {
  ...
  summaryText: 'Bei ' + formatCurrency(inv) + ' Investition und ' + formatCurrency(annualGain) + ' jährlichem Ertrag amortisiert sich das Projekt in ca. ' + pbY + ' Jahren und ' + pbM + ' Monaten. Der Gesamt-ROI nach ' + years + ' Jahren beträgt ' + formatPercent(totalRoiPct) + ' (' + formatCurrency(netProfitTotal) + ' Reingewinn).',
    ...
    workedExample: {
      ...
      inputValues: [{ label: 'Investition', value: '50.000 €' }, { label: 'Ertrag p.a.', value: '15.000 €' }, { label: 'Dauer', value: '5 Jahre' }],
      steps: ['Gesamtertrag = 5 × 15.000 € = 75.000 €', 'Nettogewinn = 75.000 € - 50.000 € = 25.000 €', 'ROI = (25.000 / 50.000) × 100 = 50,0 %', 'Amortisation = 50.000 / 15.000 = 3,33 Jahre (3 Jahre, 4 Monate)'],
      result: '50,0 % ROI (Amortisation nach 3,3 Jahren)',
    },
    faqs: [
      { question: 'Was ist ein guter ROI?', answer: 'Als allgemeiner Richtwert gilt: Ein ROI über 10 % p.a. ist solide, ab 15 % bis 20 % gilt eine Investition als hochattraktiv. Im Marketing und E-Commerce werden oft noch deutlich höhere kurzfristige ROIs erzielt.' },
      { question: 'Berücksichtigt die einfache ROI-Methode Zinsen oder Inflation?', answer: 'Nein, die einfache ROI-Berechnung ignoriert den Zeitwert des Geldes. Für exakte mehrjährige Großprojekte nutzt man stattdessen die Kapitalwertmethode (NPV) oder den internen Zinsfuß (IRR).' },
    ],
...

  {
    ...
    inputs: [
      { id: 'revenue', label: 'Umsatzerlöse', type: 'number', defaultValue: 500000, min: 1000, max: 1000000000, step: 5000, unit: '€' },
      { id: 'materialExpense', label: 'Materialaufwand / Wareneinsatz', type: 'number', defaultValue: 180000, min: 0, max: 1000000000, step: 2500, unit: '€' },
      { id: 'personnelExpense', label: 'Personalaufwand (Gehälter, Sozialabgaben)', type: 'number', defaultValue: 170000, min: 0, max: 1000000000, step: 2500, unit: '€' },
      { id: 'otherOperatingExpenses', label: 'Sonstige betriebliche Aufwendungen (Miete, Marketing)', type: 'number', defaultValue: 50000, min: 0, max: 1000000000, step: 1000, unit: '€' },
      { id: 'depreciation', label: 'Abschreibungen auf Sachanlagen & immaterielle Werte (AfA)', type: 'number', defaultValue: 25000, min: 0, max: 100000000, step: 1000, unit: '€' },
    ],
    calculateCode: `const rev = Number(inputs.revenue) || 0;
...
return {
  ...
  summaryText: 'Bei ' + formatCurrency(rev) + ' Umsatz erzielt das Unternehmen ein EBITDA von ' + formatCurrency(ebitda) + ' (EBITDA-Marge ' + formatPercent(ebitdaMargin) + ') und ein EBIT von ' + formatCurrency(ebit) + ' (EBIT-Marge ' + formatPercent(ebitMargin) + ').',
    ...
    workedExample: {
      ...
      inputValues: [{ label: 'Umsatz', value: '500.000 €' }, { label: 'Betriebskosten', value: '400.000 €' }, { label: 'AfA', value: '25.000 €' }],
      steps: ['EBITDA = 500.000 € - 400.000 € = 100.000 €', 'EBIT = 100.000 € - 25.000 € = 75.000 €', 'EBITDA-Marge = 100.000 / 500.000 = 20,0 %'],
      result: 'EBITDA: 100.000 €, EBIT: 75.000 €',
    },
    faqs: [
      { question: 'Was bedeutet das Akronym EBITDA?', answer: 'EBITDA steht für "Earnings Before Interest, Taxes, Depreciation and Amortization" – also Jahresergebnis vor Zinsen, Steuern, Abschreibungen auf Sachanlagen und immaterielle Vermögenswerte.' },
      { question: 'Warum nutzen Banken und Investoren EBITDA für Unternehmensbewertungen?', answer: 'Weil das EBITDA den reinen operativen Mittelzufluss widerspiegelt, der zur Bedienung von Krediten und für Reinvestitionen zur Verfügung steht, unabhängig davon, wie das Unternehmen bilanziert oder finanziert ist.' },
    ],
...

  {
    ...
    inputs: [
      { id: 'netIncome', label: 'Jahresüberschuss / Reingewinn nach Steuern', type: 'number', defaultValue: 60000, min: -100000000, max: 100000000, step: 2500, unit: '€' },
      { id: 'depreciation', label: 'Abschreibungen auf Anlagevermögen (AfA)', type: 'number', defaultValue: 35000, min: 0, max: 10000000, step: 1000, unit: '€' },
      { id: 'changeProvisions', label: 'Zuführung zu langfristigen Rückstellungen (z. B. Pensionen)', type: 'number', defaultValue: 10000, min: -10000000, max: 10000000, step: 500, unit: '€' },
      { id: 'revenue', label: 'Gesamtumsatz (für Cashflow-Marge)', type: 'number', defaultValue: 500000, min: 1000, max: 1000000000, step: 5000, unit: '€' },
    ],
    calculateCode: `const netInc = Number(inputs.netIncome) || 0;
const afa = Number(inputs.depreciation) || 0;
...
return {
  ...
  summaryText: 'Der operative Cashflow beträgt ' + formatCurrency(cashflow) + '. Das entspricht einer Cashflow-Umsatzrate von ' + formatPercent(cashflowMargin) + '. Dieser Betrag steht dem Unternehmen für Tilgung, Investitionen und Ausschüttungen liquide zur Verfügung.',
    ...
    workedExample: {
      ...
      inputValues: [{ label: 'Gewinn', value: '60.000 €' }, { label: 'AfA', value: '35.000 €' }, { label: 'Rückstellungen', value: '10.000 €' }],
      steps: ['Cashflow = 60.000 € + 35.000 € + 10.000 € = 105.000 €', 'Cashflow-Marge = (105.000 / 500.000) × 100 = 21,0 %'],
      result: '105.000 € operativer Cashflow (21 % Marge)',
    },
    faqs: [
      { question: 'Kann ein Unternehmen trotz hohem Gewinn insolvent werden?', answer: 'Ja! Wenn Kunden Rechnungen spät bezahlen oder viel Kapital im Warenlager gebunden ist, kann der Cashflow negativ werden. Viele insolvente Firmen waren auf dem Papier profitabel, hatten aber keine Liquidität.' },
      { question: 'Was ist der Free Cashflow?', answer: 'Der Free Cashflow ist der operative Cashflow abzüglich der getätigten Investitionsausgaben (Capex). Er beziffert das Geld, das den Eigentümern frei zur Entnahme oder für Dividendenausschüttungen verbleibt.' },
    ],
...

  {
    ...
    inputs: [
      { id: 'cashAndBanks', label: 'Flüssige Mittel (Kasse, Bankguthaben)', type: 'number', defaultValue: 30000, min: 0, max: 100000000, step: 1000, unit: '€' },
      { id: 'shortTermReceivables', label: 'Kurzfristige Forderungen (LuL)', type: 'number', defaultValue: 70000, min: 0, max: 100000000, step: 1000, unit: '€' },
      { id: 'inventories', label: 'Vorräte / Warenlager', type: 'number', defaultValue: 60000, min: 0, max: 100000000, step: 1000, unit: '€' },
      { id: 'shortTermLiabilities', label: 'Kurzfristige Verbindlichkeiten (Lieferanten, Dispo)', type: 'number', defaultValue: 80000, min: 1, max: 100000000, step: 1000, unit: '€' },
    ],
    calculateCode: `const cash = Number(inputs.cashAndBanks) || 0;
...
return {
  ...
  summaryText: 'Die Liquidität 1. Grades liegt bei ' + formatPercent(l1) + ', 2. Grades bei ' + formatPercent(l2) + ' und 3. Grades bei ' + formatPercent(l3) + '. Das Unternehmen ist ' + (l2 >= 100 ? 'solide finanziert und zahlungsfähig.' : 'gefährdet, fällige Verbindlichkeiten pünktlich zu begleichen.'),
    ...
    workedExample: {
      ...
      inputValues: [{ label: 'Bar', value: '30.000 €' }, { label: 'Forderungen', value: '70.000 €' }, { label: 'Schulden', value: '80.000 €' }],
      steps: ['L1 = (30.000 / 80.000) × 100 = 37,5 %', 'L2 = (100.000 / 80.000) × 100 = 125,0 % (Idealwert > 100 % erfüllt)', 'L3 = (160.000 / 80.000) × 100 = 200,0 %'],
      result: 'L1: 37,5 %, L2: 125 %, L3: 200 %',
    },
    faqs: [
      { question: 'Warum ist die Liquidität 2. Grades (Quick Ratio) die wichtigste Kennzahl?', answer: 'Weil Waren im Lager (3. Grad) oft nicht sofort zu Geld gemacht werden können, ohne hohe Preisnachlässe zu gewähren. Bei L2 müssen allein Kasse und offene Rechnungen alle fälligen Schulden abdecken.' },
      { question: 'Ist eine zu hohe Liquidität (z. B. 80 % Barliquidität) schädlich?', answer: 'Ja, übermäßige Barliquidität bringt kaum Zinsen und bedeutet, dass Kapital unproduktiv auf dem Girokonto liegt, anstatt gewinnbringend investiert zu werden.' },
    ],
...

  {
    ...
    inputs: [
      { id: 'currentAssets', label: 'Gesamtes Umlaufvermögen (Kasse, Forderungen, Vorräte)', type: 'number', defaultValue: 180000, min: 0, max: 1000000000, step: 2500, unit: '€' },
      { id: 'currentLiabilities', label: 'Kurzfristige Verbindlichkeiten (< 1 Jahr Laufzeit)', type: 'number', defaultValue: 110000, min: 1, max: 1000000000, step: 2500, unit: '€' },
    ],
    calculateCode: `const assets = Number(inputs.currentAssets) || 0;
...
return {
  ...
  summaryText: 'Das Unternehmen verfügt über ein Working Capital von ' + formatCurrency(wc) + ' (Working Capital Ratio ' + formatPercent(wcRatio) + '). ' + (isPositive ? 'Der laufende Geschäftsbetrieb ist durch einen soliden Puffer abgesichert.' : 'Achtung: Kurzfristige Verbindlichkeiten übersteigen das Umlaufvermögen.'),
    ...
    workedExample: {
      ...
      inputValues: [{ label: 'Umlaufvermögen', value: '180.000 €' }, { label: 'Verbindlichkeiten', value: '110.000 €' }],
      steps: ['Working Capital = 180.000 € - 110.000 € = +70.000 €', 'Ratio = (180.000 / 110.000) × 100 = 163,6 %'],
      result: '+70.000 € Working Capital (Ratio 164 %)',
    },
    faqs: [
      { question: 'Was tun bei zu hohem Working Capital?', answer: 'Ein zu hohes Working Capital bindet unnötig viel Kapital (z. B. durch riesige Lagerbestände oder zögerliches Mahnwesen bei Kunden). Ziel des Working-Capital-Managements ist es, Vorräte und Forderungslaufzeiten zu minimieren.' },
      { question: 'Gibt es Branchen mit bewusst negativem Working Capital?', answer: 'Ja! Große Lebensmitteleinzelhändler (z. B. Aldi, Edeka) haben oft ein negatives Working Capital: Kunden zahlen bar oder mit Karte sofort, während Lieferanten erst nach 60-90 Tagen bezahlt werden. Das Unternehmen finanziert sich zinslos über Lieferantenkredite!' },
    ],
...

  {
    ...
    name: 'Umsatzrentabilität Rechner (Umsatzrendite & Return on Sales ROS)',
    ...
    inputs: [
      { id: 'revenue', label: 'Gesamtumsatz (Nettoumsatzerlöse)', type: 'number', defaultValue: 1000000, min: 100, max: 1000000000, step: 10000, unit: '€' },
      { id: 'profit', label: 'Gewinn (Jahresüberschuss vor oder nach Steuern)', type: 'number', defaultValue: 80000, min: -100000000, max: 100000000, step: 2500, unit: '€' },
    ],
    calculateCode: `const rev = Number(inputs.revenue) || 1;
...
return {
  ...
  summaryText: 'Von jedem Euro Umsatz verbleiben dem Unternehmen genau ' + formatNumber(centPerEuro, 1) + ' Cent Gewinn. Die Umsatzrendite beträgt ' + formatPercent(ros) + ' (' + rating + ').',
    ...
    workedExample: {
      ...
      inputValues: [{ label: 'Umsatz', value: '1.000.000 €' }, { label: 'Gewinn', value: '80.000 €' }],
      steps: ['Umsatzrendite = (80.000 / 1.000.000) × 100 = 8,0 %', 'Entspricht 8 Cent Gewinn je 1 € Umsatz'],
      result: '8,0 % Umsatzrendite',
    },
    faqs: [
      { question: 'Welche Umsatzrendite ist normal in Deutschland?', answer: 'Im deutschen Schnitt liegt die Nettoumsatzrendite im Mittelstand bei ca. 4 % bis 7 %. Im Lebensmitteleinzelhandel oft bei nur 1,5 % bis 2 %, in der Softwarebranche dagegen häufig bei 20 % bis 35 %.' },
      { question: 'Was ist der Unterschied zwischen EBIT-Marge und Umsatzrendite?', answer: 'Die operative EBIT-Marge setzt das Betriebsergebnis vor Zinsen und Steuern ins Verhältnis zum Umsatz. Die klassische Umsatzrendite (ROS) basiert meist auf dem Jahresüberschuss nach Steuern.' },
    ],
...

  {
    ...
    name: 'Eigenkapitalrentabilität Rechner (Return on Equity ROE & Leverage)',
    ...
    inputs: [
      { id: 'equity', label: 'Bilanziertes Eigenkapital', type: 'number', defaultValue: 250000, min: 1000, max: 1000000000, step: 5000, unit: '€' },
      { id: 'netProfit', label: 'Jahresüberschuss (Reingewinn nach Steuern)', type: 'number', defaultValue: 40000, min: -100000000, max: 100000000, step: 2500, unit: '€' },
    ],
    calculateCode: `const eq = Number(inputs.equity) || 1;
...
return {
  ...
  summaryText: 'Das eingesetzte Eigenkapital von ' + formatCurrency(eq) + ' verzinst sich mit ' + formatPercent(roe) + ' pro Jahr (' + rating + ').',
    ...
    workedExample: {
      ...
      inputValues: [{ label: 'Eigenkapital', value: '250.000 €' }, { label: 'Gewinn', value: '40.000 €' }],
      steps: ['ROE = (40.000 / 250.000) × 100 = 16,0 %'],
      result: '16,0 % Eigenkapitalrendite',
    },
    faqs: [
      { question: 'Was ist der Leverage-Effekt beim Eigenkapital?', answer: 'Liegt die Gesamtkapitalrendite über dem Fremdkapitalzins, steigt die Eigenkapitalrendite durch die Aufnahme von günstigem Fremdkapital an. Umgekehrt drohen bei sinkenden Erträgen massive Verluste (negativer Hebel).' },
      { question: 'Warum ist ein extrem hoher ROE nicht immer ein Qualitätsmerkmal?', answer: 'Ein künstlich hoher ROE kann entstehen, wenn ein Unternehmen kaum Eigenkapital besitzt und hochgradig mit Schulden überladen ist, was die Insolvenzgefahr bei Krisen drastisch erhöht.' },
    ],
...

  {
    ...
    name: 'Gesamtkapitalrentabilität Rechner (Return on Assets ROA & ROI)',
    ...
    inputs: [
      { id: 'totalCapital', label: 'Gesamtkapital / Bilanzsumme (Passiva)', type: 'number', defaultValue: 1000000, min: 1000, max: 1000000000, step: 10000, unit: '€' },
      { id: 'netProfit', label: 'Jahresüberschuss (Reingewinn)', type: 'number', defaultValue: 60000, min: -100000000, max: 100000000, step: 2500, unit: '€' },
      { id: 'interestExpense', label: 'Fremdkapitalzinsen / Zinsaufwand an Banken', type: 'number', defaultValue: 20000, min: 0, max: 100000000, step: 1000, unit: '€' },
    ],
    calculateCode: `const cap = Number(inputs.totalCapital) || 1;
const profit = Number(inputs.netProfit) || 0;
...
return {
  ...
  summaryText: 'Das gesamte im Unternehmen gebundene Kapital erwirtschaftet eine Verzinsung von ' + formatPercent(roa) + ' (' + formatCurrency(totalEarnings) + ' Gesamtertrag auf ' + formatCurrency(cap) + ' Bilanzsumme).',
    ...
    workedExample: {
      ...
      inputValues: [{ label: 'Gesamtkapital', value: '1.000.000 €' }, { label: 'Gewinn', value: '60.000 €' }, { label: 'Zinsen', value: '20.000 €' }],
      steps: ['Gesamtertrag = 60.000 € + 20.000 € = 80.000 €', 'ROA = (80.000 / 1.000.000) × 100 = 8,0 %'],
      result: '8,0 % Gesamtkapitalrentabilität',
    },
    faqs: [
      { question: 'Warum addiert man die Zinsen bei der Gesamtkapitalrendite wieder hinzu?', answer: 'Weil die Zinsen der Ertrag des Fremdkapitals sind. Durch die Hinzurechnung wird die Kennzahl unabhängig davon, ob das Unternehmen mit Eigen- oder Fremdkapital finanziert ist.' },
      { question: 'Welcher ROA gilt in der Industrie als gut?', answer: 'Eine Gesamtkapitalrendite über dem marktüblichen Kreditzins (z. B. > 6 % bis 8 %) ist das Mindestziel, um den Hebel des Fremdkapitals positiv nutzen zu können.' },
    ],
    relatedSlugs: ['eigenkapitalrentabilitaet-rechner', 'umsatzrentabilitaet-rechner', 'ebit-ebitda-rechner'],
  },
];

console.log('Building business part 1 with', calcs.length, 'calculators');
fs.writeFileSync(path.join(__dirname, 'calcs-business-part1.json'), JSON.stringify(calcs, null, 2), 'utf8');
console.log('Saved calcs-business-part1.json');
