...
// Wir generieren src/data/calculators/extra/wohnenHaushalt.ts
const filePath = path.join(__dirname, '../../src/data/calculators/extra/wohnenHaushalt.ts');

const content = `import { CalculatorDefinition } from '@/types/calculator';
import { formatNumber, formatCurrency, formatPercent } from '@/lib/formatters';

export const EXTRA_WOHNEN_HAUSHALT: CalculatorDefinition[] = [
  // ==================== FINANZEN (1 ZUSÄTZLICHER ZUR KOMPLETTIERUNG AUF 25) ====================
  {
    id: 'kirchensteuer-rechner',
    slug: 'kirchensteuer-rechner',
    name: 'Kirchensteuer Rechner (8 % bzw. 9 % nach Bundesland)',
    shortName: 'Kirchensteuer berechnen',
    category: 'finanzen',
    subcategory: 'Steuern & Abgaben',
    metaTitle: 'Kirchensteuer Rechner – Kirchensteuerhöhe & Kirchenaustritt Ersparnis',
    metaDescription: 'Berechnen Sie Ihre Kirchensteuer für 2026: 8 % in Bayern und Baden-Württemberg, 9 % in den übrigen 14 Bundesländern. Inklusive Kappungsgrenzen.',
    h1: 'Kirchensteuer Rechner – Kirchensteuer & Ersparnis berechnen',
    shortDescription: 'Berechnet die genaue Kirchensteuerbelastung basierend auf Einkommensteuer und Bundesland.',
    searchKeywords: ['kirchensteuer rechner', 'kirchensteuer bayern', 'kirchenaustritt rechner ersparnis', 'kirchensteuer absetzen'],
    inputs: [
      { id: 'incomeTax', label: 'Jährliche Einkommensteuer / Lohnsteuer', type: 'number', defaultValue: 8000, min: 0, step: 250, unit: '€' },
      {
        id: 'state',
        label: 'Bundesland',
        type: 'select',
        defaultValue: 'other',
        options: [
          { value: 'by_bw', label: 'Bayern oder Baden-Württemberg (8 %)' },
          { value: 'other', label: 'Übrige 14 Bundesländer (9 %)' },
        ],
      },
    ],
    calculate: (inputs) => {
      const tax = parseFloat(inputs.incomeTax) || 8000;
      const rate = inputs.state === 'by_bw' ? 0.08 : 0.09;
      const churchTaxYear = tax * rate;
      const churchTaxMonth = churchTaxYear / 12;
      return {
        primary: { id: 'churchTaxYear', label: 'Jährliche Kirchensteuer', value: churchTaxYear, formattedValue: formatCurrency(churchTaxYear), highlight: true },
        secondary: [
          { id: 'churchTaxMonth', label: 'Monatlicher Abzug', value: churchTaxMonth, formattedValue: formatCurrency(churchTaxMonth) },
          { id: 'churchRate', label: 'Angewandter Steuersatz', value: rate * 100, formattedValue: formatPercent(rate * 100, 0) },
        ],
        summaryText: \`Bei einer Lohn- oder Einkommensteuer von \${formatCurrency(tax)} beträgt Ihre Kirchensteuer ca. \${formatCurrency(churchTaxYear)} pro Jahr (\${formatCurrency(churchTaxMonth)} monatlich).\`,
      };
    },
    formula: 'Kirchensteuer = Einkommensteuer × Steuersatz (8 % in BY/BW, sonst 9 %)',
    formulaExplanation: 'Bemessungsgrundlage der Kirchensteuer ist die festgesetzte Einkommensteuer bzw. einbehaltene Lohnsteuer (§ 51a EStG).',
    workedExample: {
      title: 'Beispiel: 8.000 € Lohnsteuer in NRW (9 %)',
      inputValues: [{ label: 'Einkommensteuer', value: '8.000 €' }, { label: 'Bundesland', value: 'NRW (9 %)' }],
      steps: ['Kirchensteuer = 8.000 € × 0,09 = 720 € pro Jahr', 'Monatlicher Abzug = 720 € / 12 = 60 €'],
      result: '720,00 € pro Jahr',
    },
    faqs: [
      { question: 'Ist Kirchensteuer steuerlich absetzbar?', answer: 'Ja, gezahlte Kirchensteuer kann im Rahmen der Einkommensteuererklärung in voller Höhe als Sonderausgabe nach § 10 EStG geltend gemacht werden.' },
      { question: 'Gilt die Kirchensteuer auch bei Kapitalerträgen?', answer: 'Ja, auch auf die 25 % Abgeltungsteuer wird Kirchensteuer erhoben, wodurch sich der Abgeltungsteuersatz leicht ermäßigt.' },
    ],
    relatedSlugs: ['brutto-netto-rechner', 'einkommensteuerrechner', 'abgeltungsteuer-rechner'],
  },
  ...
  // ==================== WOHNEN & IMMOBILIEN (20 ZUSÄTZLICHE) ====================
  {
    ...
    metaTitle: 'Mietminderung Rechner – Ersparnis bei Wohnungsmängeln nach § 536 BGB',
    ...
    faqs: [
      { question: 'Gilt die Minderung rückwirkend?', answer: 'Grundsätzlich erst ab dem Zeitpunkt, an dem der Vermieter von dem Mangel Kenntnis erlangt hat (Mängelanzeige nach § 536c BGB).' },
      { question: 'Muss die Brutto- oder Kaltmiete gekürzt werden?', answer: 'Der Bundesgerichtshof hat entschieden, dass immer von der Bruttowarmmiete (inkl. Nebenkostenvorauszahlung) ausgegangen wird.' },
    ],
    relatedSlugs: ['mietpreisbremse-rechner', 'mietkaution-rechner', 'mietrendite-rechner'],
  },

  {
    ...
    category: 'wohnen-immobilien',
    subcategory: 'Miete & Mietrecht',
    metaTitle: 'Staffelmiete Rechner – Zukünftige Mieterhöhungen & Staffeln kalkulieren',
    ...
    faqs: [
      { question: 'Darf der Vermieter neben der Staffelmiete noch modernisieren?', answer: 'Nein, während der Laufzeit einer Staffelmiete ist eine Mieterhöhung nach § 559 BGB (Modernisierung) gesetzlich ausgeschlossen.' },
      { question: 'Gilt bei Staffelmiete die Mietpreisbremse?', answer: 'Ja, auch jede einzelne Staffel muss bei Wiedervermietung in Gebieten mit Mietpreisbremse die zulässige Höchstgrenze beachten.' },
    ],
    relatedSlugs: ['indexmiete-rechner', 'mietpreisbremse-rechner', 'kauf-vs-miete-rechner'],
  },

  {
    ...
    category: 'wohnen-immobilien',
    subcategory: 'Miete & Mietrecht',
    metaTitle: 'Indexmiete Rechner – Mieterhöhung nach VPI Statistisches Bundesamt',
    ...
    faqs: [
      { question: 'Wann wird die Erhöhung wirksam?', answer: 'Die Erhöhung wird mit Beginn des übernächsten Monats nach Zugang der Erklärung in Textform wirksam.' },
      { question: 'Kann eine Indexmiete auch sinken?', answer: 'Ja, sollte der Verbraucherpreisindex fallen (Deflation), hat der Mieter einen rechtlichen Anspruch auf Senkung der Miete.' },
    ],
    relatedSlugs: ['staffelmiete-rechner', 'mietpreisbremse-rechner', 'inflationsrechner'],
  },

  {
    ...
    metaTitle: 'Wohnflächenrechner WoFlV – Wohnfläche exakt nach Vorschrift berechnen',
    ...
    faqs: [
      { question: 'Was ist der Unterschied zwischen WoFlV und DIN 277?', answer: 'Die DIN 277 rechnet Grundflächen zu 100 % ein (Nutzfläche). Die WoFlV ist bindend für Wohnraummietverträge und berücksichtigt Schrägen nur anteilig.' },
      { question: 'Ab welcher Flächenabweichung darf die Miete gemindert werden?', answer: 'Weicht die tatsächliche Wohnfläche um mehr als 10 % von der im Mietvertrag angegebenen Fläche ab, liegt ein Mangel vor.' },
    ],
    relatedSlugs: ['grundsteuer-reform-rechner', 'mietpreisbremse-rechner', 'kauf-vs-miete-rechner'],
  },

  {
    ...
    category: 'wohnen-immobilien',
    subcategory: 'Immobilienkauf & Finanzierung',
    metaTitle: 'Grundsteuer Rechner – Grundsteuer nach neuer Reform 2025/2026 berechnen',
    ...
    shortDescription: 'Berechnet die neue Grundsteuer nach dem 3-Stufen-Verfahren für Wohngrundstücke.',
    ...
    faqs: [
      { question: 'Wann ist die Grundsteuer fällig?', answer: 'Die Grundsteuer wird grundsätzlich je zu einem Viertel am 15. Februar, 15. Mai, 15. August und 15. November fällig.' },
      { question: 'Kann die Grundsteuer auf den Mieter umgelegt werden?', answer: 'Ja, als laufende öffentliche Last des Grundstücks ist sie über die Nebenkostenabrechnung voll umlagefähig (§ 2 Nr. 1 BetrKV).' },
    ],
    relatedSlugs: ['grunderwerbsteuer-rechner', 'kaufnebenkosten-rechner', 'hausgeld-rechner'],
  },

  {
    ...
    category: 'wohnen-immobilien',
    subcategory: 'Immobilienkauf & Finanzierung',
    metaTitle: 'Spekulationssteuer Rechner Immobilien – Steuer bei Verkauf vor 10 Jahren',
    ...
    faqs: [
      { question: 'Gilt die 10-Jahres-Frist auch bei Eigennutzung?', answer: 'Nein, wurde die Immobilie im Jahr der Veräußerung und in den beiden vorangegangenen Jahren selbst bewohnt, ist der Gewinn steuerfrei.' },
      { question: 'Ab wann beginnt die 10-Jahres-Frist zu laufen?', answer: 'Entscheidend sind die Notardaten der beiden Kaufverträge (Kauf und Verkauf), nicht der Grundbucheintrag oder Übergabetag.' },
    ],
...

  {
    ...
    category: 'wohnen-immobilien',
    subcategory: 'Immobilienkauf & Finanzierung',
    metaTitle: 'Maklerprovision Rechner – Maklerkosten für Käufer und Verkäufer',
    ...
    faqs: [
      { question: 'Darf der Makler vom Käufer mehr verlangen als vom Verkäufer?', answer: 'Nein, Vereinbarungen, die den Käufer zu einer höheren Provisionszahlung als den Verkäufer verpflichten, sind nach § 656c BGB unwirksam.' },
      { question: 'Gilt das Gesetz auch bei Mehrfamilienhäusern oder Grundstücken?', answer: 'Nein, die gesetzliche Halbteilung gilt nur für Einfamilienhäuser und Eigentumswohnungen bei privaten Käufern.' },
    ],
...

  {
    ...
    category: 'wohnen-immobilien',
    subcategory: 'Immobilienkauf & Finanzierung',
    metaTitle: 'Grunderwerbsteuer Rechner – Steuersätze aller 16 Bundesländer 2026',
    ...
    shortDescription: 'Ermittelt die exakte Grunderwerbsteuer für Ihren Immobilien- oder Grundstückskauf.',
    ...
    faqs: [
      { question: 'Kann man bewegliche Gegenstände aus der Steuer herausrechnen?', answer: 'Ja, mitverkaufte Möbel, Einbauküchen oder Photovoltaikanlagen können im Kaufvertrag gesondert ausgewiesen werden und unterliegen nicht der Grunderwerbsteuer.' },
      { question: 'Wann erhält man die Unbedenklichkeitsbescheinigung?', answer: 'Das Finanzamt stellt die Unbedenklichkeitsbescheinigung aus, sobald die Grunderwerbsteuer vollständig bezahlt wurde. Erst dann erfolgt der Grundbucheintrag.' },
    ],
    relatedSlugs: ['kaufnebenkosten-rechner', 'notar-grundbuch-kosten-rechner', 'maklerprovision-rechner'],
  },

  {
    ...
    category: 'wohnen-immobilien',
    subcategory: 'Immobilienkauf & Finanzierung',
    metaTitle: 'Notarkosten Rechner Immobilienkauf – Notar- & Grundbuchgebühren GNotKG',
    ...
    shortDescription: 'Kalkuliert die gesetzlichen Gebühren für Notarbeurkundung und Grundbucheintragung.',
    ...
    faqs: [
      { question: 'Kann man Notarkosten verhandeln?', answer: 'Nein, Notare sind nach dem GNotKG gesetzlich verpflichtet, die exakten Gebühren zu erheben. Rabatte sind berufsrechtlich unzulässig.' },
      { question: 'Wer zahlt den Notar beim Immobilienkauf?', answer: 'Üblicherweise trägt der Käufer die Notar- und Grundbuchkosten für den Kaufvertrag und seine Grundschuld.' },
    ],
...

  {
    ...
    metaTitle: 'Instandhaltungsrücklage Rechner – Peterssche Formel & WEG-Rücklagen',
    ...
    faqs: [
      { question: 'Wer beschließt die Höhe der Rücklage in der WEG?', answer: 'Die Eigentümergemeinschaft beschließt die Höhe der Erhaltungsrücklage im Rahmen des Wirtschaftsplans mit einfacher Mehrheit (§ 19 WEG).' },
      { question: 'Geht die Rücklage beim Wohnungsverkauf verloren?', answer: 'Nein, der Anteil an der Instandhaltungsrücklage verbleibt beim Eigentum und geht automatisch auf den Käufer über.' },
    ],
    relatedSlugs: ['hausgeld-rechner', 'kaufnebenkosten-rechner', 'mietrendite-rechner'],
  },

  {
    ...
    category: 'wohnen-immobilien',
    subcategory: 'Miete & Mietrecht',
    metaTitle: 'Modernisierungsumlage Rechner – Mieterhöhung nach § 559 BGB berechnen',
    ...
    faqs: [
      { question: 'Müssen Fördermittel abgezogen werden?', answer: 'Ja, staatliche Zuschüsse (z. B. KfW-Förderung oder BAFA) müssen vor der Berechnung von den Gesamtkosten abgezogen werden (§ 559a BGB).' },
      { question: 'Wann tritt die Mieterhöhung in Kraft?', answer: 'Die Miete erhöht sich mit Beginn des dritten Monats nach Zugang der formal ordnungsgemäßen Mieterhöhungserklärung.' },
    ],
    relatedSlugs: ['indexmiete-rechner', 'staffelmiete-rechner', 'mietpreisbremse-rechner'],
  },

  {
    ...
    category: 'wohnen-immobilien',
    subcategory: 'Miete & Mietrecht',
    metaTitle: 'Mietkaution Rechner – Zinsen, Zinseszins & Auszahlung nach Auszug',
    ...
    faqs: [
      { question: 'Darf der Mieter die Kaution in 3 Raten zahlen?', answer: 'Ja, nach § 551 Abs. 2 BGB ist der Mieter gesetzlich berechtigt, die Kaution in drei gleichen Monatsraten zu leisten.' },
      { question: 'Wie lange darf der Vermieter die Kaution nach Auszug einbehalten?', answer: 'Der Vermieter hat eine angemessene Prüfungs- und Überlegungsfrist von üblicherweise 3 bis maximal 6 Monaten.' },
    ],
    relatedSlugs: ['mietpreisbremse-rechner', 'mietminderung-rechner', 'kauf-vs-miete-rechner'],
  },

  {
    ...
    shortDescription: 'Kalkuliert die monatliche Hausgeldzahlung und trennt Vermieterkosten von Mieterkosten.',
    ...
    faqs: [
      { question: 'Was ist im nicht umlagefähigen Teil enthalten?', answer: 'Typischerweise die Verwaltergebühr, Bankkontoführungskosten sowie die Zuführung zur Instandhaltungsrücklage.' },
      { question: 'Wie unterscheidet sich Hausgeld von Wohngeld?', answer: 'Im WEG-Recht wird die Vorauszahlung der Eigentümer Hausgeld oder Wohngeld genannt. Davon zu unterscheiden ist das staatliche Wohngeld als Sozialleistung.' },
    ],
    relatedSlugs: ['instandhaltungsruecklage-rechner', 'mietrendite-rechner', 'grundsteuer-reform-rechner'],
  },

  {
    ...
    category: 'wohnen-immobilien',
    subcategory: 'Immobilienkauf & Finanzierung',
    metaTitle: 'AfA Rechner Immobilien – Gebäudeabschreibung 2 %, 2,5 % oder 3 %',
    metaDescription: 'Berechnen Sie die steuerliche Gebäudeabschreibung (AfA) für vermietete Immobilien nach § 7 Abs. 4 EStG: 2 % (ab 1925), 2,5 % (vor 1925) oder 3 % (Neubau ab 2023).',
    ...
    faqs: [
      { question: 'Gilt die AfA auch für selbst genutztes Wohneigentum?', answer: 'Nein, die lineare Gebäude-AfA steht nur Vermietern zur Erzielung von Einkünften aus Vermietung und Verpachtung zu.' },
      { question: 'Wie weise ich einen höheren Gebäudeanteil beim Finanzamt nach?', answer: 'Über die offizielle Kaufpreisaufteilungshilfe des Bundesfinanzministeriums oder durch ein qualifiziertes Bausachverständigen-Gutachten.' },
    ],
    relatedSlugs: ['spekulationssteuer-immobilien-rechner', 'mietrendite-rechner', 'kaufnebenkosten-rechner'],
  },

  {
    ...
    category: 'wohnen-immobilien',
    subcategory: 'Immobilienkauf & Finanzierung',
    metaTitle: 'Mietrendite Rechner – Brutto- & Nettomietrendite für Kapitalanleger',
    ...
    shortDescription: 'Vergleicht Brutto- und Nettomietrendite zur objektiven Bewertung von Kapitalanlage-Immobilien.',
    ...
    faqs: [
      { question: 'Ab welcher Nettomietrendite lohnt sich eine Immobilie?', answer: 'In der Regel gilt eine Nettomietrendite ab 3,5 % bis 4,5 % je nach Lage und Zinsumfeld als solides Investment.' },
      { question: 'Was besagt der Vervielfältiger (Kaufpreisfaktor)?', answer: 'Er gibt an, nach wie vielen Jahren die reinen Mieteinnahmen den Kaufpreis decken (z. B. 20-fache bis 25-fache Jahreskaltmiete).' },
    ],
    relatedSlugs: ['kauf-vs-miete-rechner', 'abschreibung-immobilien-rechner', 'hausgeld-rechner'],
  },

  {
    id: 'vorfaelligkeitsentschaedigung-rechner',
    slug: 'vorfaelligkeitsentschaedigung-rechner',
    name: 'Vorfälligkeitsentschädigung Rechner (Aktiv-Passiv-Methode)',
    shortName: 'Vorfälligkeitsentschädigung',
    category: 'wohnen-immobilien',
    subcategory: 'Immobilienkauf & Finanzierung',
    metaTitle: 'Vorfälligkeitsentschädigung Rechner – Bankgebühren bei Darlehenskündigung',
    metaDescription: 'Schätzen Sie die Vorfälligkeitsentschädigung bei vorzeitiger Kündigung Ihrer Baufinanzierung nach der vom BGH anerkannten Aktiv-Passiv-Methode.',
    h1: 'Vorfälligkeitsentschädigung Rechner – Kosten bei Kreditkündigung',
    shortDescription: 'Schätzt die Entschädigung der Bank bei vorzeitiger Ablösung eines Immobiliendarlehens.',
    searchKeywords: ['vorfaelligkeitsentschaedigung rechner', 'baufinanzierung vorzeitig kuendigen kosten', 'aktiv passiv methode bgh', 'schadensersatz bank zinsverlust'],
    inputs: [
    ...
    faqs: [
      { question: 'Kann ich das Darlehen nach 10 Jahren kostenlos kündigen?', answer: 'Ja! Nach § 489 Abs. 1 Nr. 2 BGB hat jeder Darlehensnehmer nach 10 Jahren Vollaufzeit ein gesetzliches Kündigungsrecht mit 6 Monaten Frist – ohne Vorfälligkeitsentschädigung.' },
      { question: 'Muss die Entschädigung bei Immobilienverkauf gezahlt werden?', answer: 'Ja, wenn Sie die Immobilie verkaufen, hat die Bank Anspruch auf Ersatz des Zinsschadens.' },
    ],
...

  {
    ...
    category: 'wohnen-immobilien',
    subcategory: 'Miete & Mietrecht',
    metaTitle: 'Abstandszahlung Rechner – Zeitwert für Einbauküche & Möbel berechnen',
    ...
    faqs: [
      { question: 'Darf der Vormieter den Mietvertrag an eine Abstandszahlung koppeln?', answer: 'Nein! Eine Kopplung von Mietvertragsabschluss an den Kauf überteuerter Möbel ist nach § 4a WoVermittG sittenwidrig und gesetzlich verboten.' },
      { question: 'Haben Nachmieter Anspruch auf Kaufbelege?', answer: 'Ja, um den Zeitwert überprüfen zu können, sollte die Originalrechnung vorgelegt werden.' },
    ],
    relatedSlugs: ['mietkaution-zinsen-rechner', 'warmmiete-zu-kaltmiete-rechner', 'mietpreisbremse-rechner'],
  },

  {
    ...
    category: 'wohnen-immobilien',
    subcategory: 'Miete & Mietrecht',
    metaTitle: 'Warmmiete zu Kaltmiete Rechner – Nebenkosten & Kaltmiete ermitteln',
    ...
    shortDescription: 'Schlüsselt Bruttowarmmieten in Nettokaltmiete und Nebenkostenvorauszahlung auf.',
    ...
    faqs: [
      { question: 'Gehören Stromkosten in die Warmmiete?', answer: 'In der Regel nein. Haushaltsstrom schließt der Mieter fast immer direkt mit dem Energieversorger ab.' },
      { question: 'Was passiert mit zu viel gezahlten Nebenkosten?', answer: 'Der Vermieter muss jährlich eine Abrechnung erstellen. Guthaben werden dem Mieter erstattet.' },
    ],
    relatedSlugs: ['mietpreisbremse-rechner', 'mietminderung-rechner', 'stromkosten-rechner'],
  },

  {
    ...
    category: 'wohnen-immobilien',
    subcategory: 'Miete & Mietrecht',
    metaTitle: 'Untermiete Rechner – Mietanteil für WG-Zimmer & Zuschlag nach § 553 BGB',
    ...
    shortDescription: 'Kalkuliert die Untermiete für WG-Zimmer inklusive Gemeinschaftsflächen und Möblierungszuschlag.',
    ...
    faqs: [
      { question: 'Muss der Hauptvermieter der Untervermietung zustimmen?', answer: 'Ja, nach § 540 und § 553 BGB ist die Erlaubnis des Vermieters erforderlich. Liegt ein berechtigtes Interesse vor, darf er sie nicht grundlos verweigern.' },
      { question: 'Wer haftet für Schäden des Untermieters?', answer: 'Gegenüber dem Eigentümer haftet immer der Hauptmieter in vollem Umfang für alle Handlungen und Schäden des Untermieters.' },
    ],
...

  {
    ...
    category: 'wohnen-immobilien',
    subcategory: 'Immobilienkauf & Finanzierung',
    metaTitle: 'Erbbauzins Rechner – Jährlichen Erbbauzins nach Bodenwert berechnen',
    ...
    shortDescription: 'Berechnet den Erbbauzins auf Basis des Bodenrichtwerts für Erbpacht-Grundstücke.',
    ...
    faqs: [
      { question: 'Kann der Erbbauzins erhöht werden?', answer: 'Ja, Erbbauverträge enthalten fast immer Wertsicherungsklauseln, die an den Verbraucherpreisindex gekoppelt sind.' },
      { question: 'Was passiert nach Ablauf des Erbbaurechts?', answer: 'Das Gebäude fällt an den Grundstückseigentümer zurück (Heimfall). Dafür erhält der Erbbauberechtigte eine gesetzliche Entschädigung von mind. zwei Dritteln des Verkehrswerts.' },
    ],
    relatedSlugs: ['grundsteuer-reform-rechner', 'kauf-vs-miete-rechner', 'kaufnebenkosten-rechner'],
  },

  {
    ...
    category: 'wohnen-immobilien',
    subcategory: 'Immobilienkauf & Finanzierung',
    metaTitle: 'Kaufpreisfaktor Rechner – Vervielfältiger für Immobilienbewertung',
    ...
    shortDescription: 'Berechnet das Verhältnis von Kaufpreis zu Jahresnettokaltmiete.',
    ...
    faqs: [
      { question: 'Welcher Faktor gilt als gut?', answer: 'Faktoren zwischen 18 und 24 gelten in vielen deutschen Städten als solide. In Metropolen wie München oder Frankfurt liegen die Faktoren oft über 30.' },
      { question: 'Bezieht der Faktor Kaufnebenkosten mit ein?', answer: 'Nein, der klassische Makler- und Marktfaktor vergleicht den reinen Angebotspreis mit der Jahreskaltmiete.' },
    ],
    relatedSlugs: ['mietrendite-brutto-netto-rechner', 'kauf-vs-miete-rechner', 'kaufnebenkosten-rechner'],
  },
  ...
  // ==================== HAUSHALT & ENERGIE (21 ZUSÄTZLICHE) ====================
  {
    ...
    metaTitle: 'Stromkosten Rechner Geräte – Watt in kWh & Stromkosten pro Jahr',
    ...
    faqs: [
      { question: 'Wie finde ich die Watt-Zahl meines Geräts heraus?', answer: 'Die maximale Leistungsaufnahme in Watt steht auf dem Typenschild auf der Rück- oder Unterseite des Geräts.' },
      { question: 'Verbrauchen Geräte im Standby-Modus Strom?', answer: 'Ja, viele Geräte ziehen auch im Standby zwischen 0,5 und 15 Watt dauerhaft Strom, was sich übers Jahr summiert.' },
    ],
    relatedSlugs: ['stromkosten-rechner', 'gasverbrauch-kwh-m3-rechner', 'led-sparrechner'],
  },

  {
    ...
    metaTitle: 'Gasverbrauch m³ in kWh Umrechner – Formel & Gaskosten berechnen',
    ...
    shortDescription: 'Wandelt abgelesene Kubikmeter vom Gaszähler in verrechnete Kilowattstunden um.',
    ...
    faqs: [
      { question: 'Wo finde ich Brennwert und Zustandszahl?', answer: 'Beide Werte stehen verbindlich auf Ihrer letzten jährlichen Gasrechnung des Versorgers.' },
      { question: 'Wie viel Gas verbraucht ein Einfamilienhaus im Schnitt?', answer: 'Ein durchschnittliches Einfamilienhaus verbraucht je nach Dämmstandard zwischen 15.000 und 25.000 kWh Gas pro Jahr.' },
    ],
    relatedSlugs: ['stromkosten-geraete-rechner', 'waermepumpe-stromkosten-rechner', 'heizkostenvergleich-rechner'],
  },

  {
    ...
    category: 'haushalt-energie',
    subcategory: 'Strom & Sparen',
    metaTitle: 'Balkonkraftwerk Rechner – 800W Ertrag, Ersparnis & Amortisation',
    ...
    shortDescription: 'Ermittelt jährlichen Solarertrag und Geldersparnis für Mini-Photovoltaikanlagen.',
    ...
    faqs: [
      { question: 'Fällt für Balkonkraftwerke Mehrwertsteuer an?', answer: 'Nein, nach § 12 Abs. 3 UStG gilt der Nullsteuersatz (0 % MwSt.) für Photovoltaikanlagen und Balkonkraftwerke.' },
      { question: 'Darf der Vermieter ein Balkonkraftwerk verbieten?', answer: 'Nach der Gesetzesänderung im Miet- und WEG-Recht haben Mieter und Wohnungseigentümer einen Rechtsanspruch auf die Genehmigung eines Balkonkraftwerks.' },
    ],
    relatedSlugs: ['photovoltaik-amortisation-rechner', 'stromkosten-geraete-rechner', 'stromkosten-rechner'],
  },

  {
    ...
    category: 'haushalt-energie',
    subcategory: 'Strom & Sparen',
    metaTitle: 'Photovoltaik Rechner – PV-Ertrag, Batteriespeicher & Einspeisevergütung',
    ...
    shortDescription: 'Ganzheitliche Wirtschaftlichkeitsrechnung für Photovoltaik-Dachanlagen.',
    ...
    faqs: [
      { question: 'Lohnt sich ein Batteriespeicher?', answer: 'Ein Speicher erhöht den Eigenverbrauch von ca. 30 % auf bis zu 70 %, wodurch weniger teurer Netzstrom zugekauft werden muss.' },
      { question: 'Müssen PV-Einnahmen versteuert werden?', answer: 'Nein, PV-Anlagen auf Einfamilienhäusern bis 30 kWp sind einkommensteuerfrei (§ 3 Nr. 72 EStG).' },
    ],
    relatedSlugs: ['balkonkraftwerk-ertrag-rechner', 'waermepumpe-stromkosten-rechner', 'e-auto-ladekosten-zuhause-rechner'],
  },

  {
    ...
    category: 'haushalt-energie',
    subcategory: 'Heizung & Gas',
    metaTitle: 'Wärmepumpen Stromkosten Rechner – JAZ & Heizstromkosten berechnen',
    ...
    shortDescription: 'Ermittelt Strombedarf und Betriebskosten von Luft-Wasser- und Sole-Wärmepumpen.',
    ...
    faqs: [
      { question: 'Was ist der Unterschied zwischen COP und JAZ?', answer: 'Der COP ist ein Labor-Momentanwert unter standardisierten Prüfbedingungen. Die JAZ misst die reale Effizienz im tatsächlichen Betrieb über ein ganzes Jahr.' },
      { question: 'Gibt es spezielle Wärmepumpen-Stromtarife?', answer: 'Ja, viele Versorger bieten günstigere Heizstromtarife an, wenn ein separater unterbrechbarer Zähler nach § 14a EnWG installiert ist.' },
    ],
    relatedSlugs: ['heizkostenvergleich-rechner', 'gasverbrauch-kwh-m3-rechner', 'photovoltaik-amortisation-rechner'],
  },

  {
    ...
    category: 'haushalt-energie',
    subcategory: 'Heizung & Gas',
    metaTitle: 'Heizkostenvergleich Rechner – Gas, Öl, Pellets & Wärmepumpe im Vergleich',
    ...
    shortDescription: 'Vergleicht die reinen Brennstoff- und Verbrauchskosten verschiedener Heiztechnologien.',
    ...
    faqs: [
      { question: 'Berücksichtigt der Rechner Wartungs- und Schornsteinfegerkosten?', answer: 'Hier werden rein die Brennstoffverbrauchskosten verglichen. Wärmepumpen haben zusätzlich geringere Schornsteinfeger- und Wartungskosten als Verbrennungsheizungen.' },
      { question: 'Wie wirkt sich eine eigene PV-Anlage auf die Wärmepumpe aus?', answer: 'Selbst erzeugter Solarstrom senkt die effektiven Stromkosten der Wärmepumpe nochmals beträchtlich.' },
    ],
    relatedSlugs: ['waermepumpe-stromkosten-rechner', 'gasverbrauch-kwh-m3-rechner', 'oelheizung-verbrauch-rechner'],
  },

  {
    ...
    category: 'haushalt-energie',
    subcategory: 'Heizung & Gas',
    metaTitle: 'Warmwasserkosten Rechner – Durchlauferhitzer & Duschkosten pro Jahr',
    ...
    faqs: [
      { question: 'Wie viel spart ein Sparduschkopf?', answer: 'Ein wassersparender Duschkopf senkt den Durchfluss von ca. 14 Liter/min auf 6 bis 8 Liter/min und spart damit rund 40 % bis 50 % Energie und Wasser.' },
      { question: 'Ist zentrale Warmwasserbereitung billiger als Durchlauferhitzer?', answer: 'Ja, Gas, Öl oder Wärmepumpen erzeugen Nutzwärme in der Regel deutlich günstiger pro kWh als Haushaltsstrom.' },
    ],
...

  {
    ...
    category: 'haushalt-energie',
    subcategory: 'Strom & Sparen',
    metaTitle: 'Kühlschrank Stromkosten Rechner – Verbrauch & Neukauf-Vergleich',
    ...
    faqs: [
      { question: 'Welche Temperatur ist für den Kühlschrank optimal?', answer: 'Verbraucherzentralen empfehlen eine Temperatur von 7 °C im oberen Fach des Kühlschranks und -18 °C im Gefrierschrank.' },
      { question: 'Was bedeuten die neuen Energieeffizienzklassen A bis G?', answer: 'Seit März 2021 wurden die Plus-Klassen (A+++) abgeschafft. Die Skala reicht wieder von A (höchste Effizienz) bis G.' },
    ],
...

  {
    ...
    category: 'haushalt-energie',
    subcategory: 'Strom & Sparen',
    metaTitle: 'Waschmaschinen Kosten Rechner – Strom & Wasser pro Waschgang berechnen',
    ...
    faqs: [
      { question: 'Wie viel Strom spart das Waschen bei niedrigeren Temperaturen?', answer: 'Bei 30 °C verbraucht eine moderne Waschmaschine oft nur ein Drittel des Stroms im Vergleich zum 60-Grad-Programm.' },
      { question: 'Lohnt sich das Eco-Programm wirklich?', answer: 'Ja, obwohl das Eco-Programm länger dauert, wird das Wasser weniger stark erhitzt, was den Energiebedarf spürbar senkt.' },
    ],
...

  {
    ...
    category: 'haushalt-energie',
    subcategory: 'Strom & Sparen',
    metaTitle: 'Wäschetrockner Rechner – Stromkosten Wärmepumpentrockner vs. Kondenstrockner',
    ...
    shortDescription: 'Vergleicht den Stromverbrauch moderner Wärmepumpentrockner mit alten Kondenstrocknern.',
    ...
    faqs: [
      { question: 'Lohnt sich der Aufpreis für einen Wärmepumpentrockner?', answer: 'Ja, schon bei 2 bis 3 Trocknungen pro Woche hat sich der Mehrpreis des Wärmepumpentrockners meist in 2 bis 3 Jahren amortisiert.' },
      { question: 'Wie kann man Trockner-Strom sparen?', answer: 'Wäsche vorher in der Waschmaschine mit maximaler Schleuderdrehzahl (z. B. 1.400 oder 1.600 U/min) vorschleudern.' },
    ],
...

  {
    ...
    category: 'haushalt-energie',
    subcategory: 'Strom & Sparen',
    metaTitle: 'Spülmaschinen Kosten Rechner – Kosten pro Spülgang vs. Handwäsche',
    ...
    faqs: [
      { question: 'Sollte man das Geschirr vor dem Einräumen abspülen?', answer: 'Nein, grobe Reste mit der Gabel in den Müll streifen genügt. Vorspülen unter fließendem Wasser verschwendet unnötig Wasser und Energie.' },
      { question: 'Lohnt sich der Warmwasseranschluss der Spülmaschine?', answer: 'Wenn das Warmwasser über eine Wärmepumpe oder Solarthermie erzeugt wird, lohnt sich der Warmwasseranschluss meist.' },
    ],
...

  {
    ...
    category: 'haushalt-energie',
    subcategory: 'Strom & Sparen',
    metaTitle: 'Fernseher Stromkosten Rechner – TV-Stromverbrauch & Standby-Kosten',
    ...
    shortDescription: 'Ermittelt die Stromkosten für Fernsehgeräte verschiedener Größen und Display-Technologien.',
    ...
    faqs: [
      { question: 'Verbrauchen OLED-Fernseher mehr Strom als LCD/LED?', answer: 'OLED-Fernseher verbrauchen bei hellen Bildern mehr Strom, bei dunklen Filmszenen hingegen sehr wenig, da schwarze Pixel komplett ausgeschaltet werden.' },
      { question: 'Wie viel Strom zieht HDR (High Dynamic Range)?', answer: 'HDR-Wiedergabe erhöht die Displayhelligkeit und kann den Stromverbrauch um bis zu 40 % bis 60 % gegenüber Standard-SDR steigern.' },
    ],
...

  {
    ...
    shortDescription: 'Vergleicht den Stromverbrauch traditioneller Glühlampen mit energiesparenden LED-Lampen.',
    ...
    faqs: [
      { question: 'Wie lange hält eine LED-Lampe im Schnitt?', answer: 'Qualitäts-LEDs haben eine durchschnittliche Nennlebensdauer von 15.000 bis 25.000 Stunden, was bei 3 Stunden pro Tag rund 15 bis 20 Jahren entspricht.' },
      { question: 'Welche Lichtfarbe entspricht warmweißem Glühbirnenlicht?', answer: 'Eine Farbtemperatur von 2.700 Kelvin entspricht exakt dem behaglich-warmen Licht der klassischen Glühlampe.' },
    ],
    relatedSlugs: ['stromkosten-geraete-rechner', 'stromkosten-rechner', 'balkonkraftwerk-ertrag-rechner'],
  },

  {
    ...
    faqs: [
      { question: 'Wofür wird im Haushalt das meiste Wasser verbraucht?', answer: 'Rund 36 % entfallen auf Körperpflege (Baden & Duschen), gefolgt von 27 % für die Toilettenspülung und 12 % für das Wäschewaschen.' },
      { question: 'Warum ist Abwasser meist teurer als Frischwasser?', answer: 'Die Reinigung in Klärwerken und die Instandhaltung des Kanalisationsnetzes erfordern hohen technischen Aufwand und Energie.' },
    ],
...

  {
    ...
    shortDescription: 'Schätzt den jährlichen Heizölbedarf nach Quadratmetern und energetischem Zustand.',
    ...
    faqs: [
      { question: 'Gilt für Heizöl auch der CO2-Preis?', answer: 'Ja, nach dem Brennstoffemissionshandelsgesetz (BEHG) ist im Heizölpreis ein nationaler CO2-Preis enthalten, der schrittweise ansteigt.' },
      { question: 'Wann ist der beste Zeitpunkt zum Heizölkauf?', answer: 'Preise schwanken mit dem Weltölmarkt. Häufig sind die Preise im Frühjahr oder Sommer bei geringerer Nachfrage günstiger als im Winter.' },
    ],
    relatedSlugs: ['heizkostenvergleich-rechner', 'gasverbrauch-kwh-m3-rechner', 'waermepumpe-stromkosten-rechner'],
  },

  {
    ...
    faqs: [
      { question: 'Wie groß muss ein Pelletlager sein?', answer: 'Pro Tonne Pellets rechnet man einschließlich Leerraum ca. 1,5 Kubikmeter Lagerraum. Für 4 Tonnen benötigt man also etwa 6 m³ Raum.' },
      { question: 'Gilt für Holzpellets die CO2-Steuer?', answer: 'Da Pellets aus nachwachsenden Holzresten hergestellt werden, unterliegen sie nicht dem nationalen CO2-Preis nach BEHG.' },
    ],
...

  {
    ...
    category: 'haushalt-energie',
    subcategory: 'Strom & Sparen',
    metaTitle: 'Klimaanlage Stromkosten Rechner – Split-Gerät vs. Mobiles Klimagerät',
    ...
    shortDescription: 'Ermittelt Stromverbrauch und Kosten für Klimageräte an heißen Sommertagen.',
    ...
    faqs: [
      { question: 'Warum sind mobile Klimageräte so ineffizient?', answer: 'Weil durch den Abluftschlauch am gekippten Fenster warme Außenluft zurück in den Raum strömt (Unterdruck-Effekt).' },
      { question: 'Kann eine Split-Klimaanlage im Winter auch heizen?', answer: 'Ja, moderne Inverter-Klimaanlagen sind Luft-Luft-Wärmepumpen und können in der Übergangszeit sehr effizient heizen.' },
    ],
...

  {
    ...
    category: 'haushalt-energie',
    subcategory: 'Strom & Sparen',
    metaTitle: 'E-Auto Ladekosten Rechner – Wallbox Zuhause vs. Schnelllader unterwegs',
    ...
    faqs: [
      { question: 'Wie hoch sind die Ladeverluste bei einer Wallbox?', answer: 'Typischerweise liegen die Ladeverluste einer 11-kW-Wallbox bei ca. 5 % bis 10 %, an einer Haushaltssteckdose (Schuko) oft bei 15 % bis 25 %.' },
      { question: 'Kann man ein E-Auto mit PV-Überschuss laden?', answer: 'Ja, mit einer PV-gesteuerten Wallbox können Sie das Auto direkt mit eigenem Solarstrom für nur 8 bis 10 Cent/kWh laden.' },
    ],
    relatedSlugs: ['photovoltaik-amortisation-rechner', 'stromkosten-geraete-rechner', 'stromkosten-rechner'],
  },

  {
    ...
    category: 'haushalt-energie',
    subcategory: 'Heizung & Gas',
    metaTitle: 'CO2-Kostenaufteilung Rechner – Stufenmodell Vermieter & Mieter CO2KostAufG',
    ...
    faqs: [
      { question: 'Muss der Vermieter die Berechnung in der Nebenkostenabrechnung belegen?', answer: 'Ja, die Einstufung in das Stufenmodell und der spezifische CO2-Ausstoß müssen transparent in der Heizkostenabrechnung aufgeführt sein.' },
      { question: 'Gilt das Gesetz auch bei Gasetagenheizungen?', answer: 'Ja, bei Gasetagenheizungen fordert der Mieter den Vermieteranteil nach Vorliegen der Gasrechnung vom Vermieter zurück.' },
    ],
...

  {
    ...
    metaTitle: 'Lebensmittelbudget Rechner – Monatliche Essenskosten & Haushaltsgeld',
    ...
    shortDescription: 'Kalkuliert realistische Monats- und Wochenausgaben für Ernährung und Einkäufe.',
    ...
    faqs: [
      { question: 'Sind Restaurantbesuche im Budget enthalten?', answer: 'Nein, dieses Budget deckt Lebensmittel, Getränke und Basishygieneartikel für das Kochen und Essen zuhause ab.' },
      { question: 'Wie kann man das Lebensmittelbudget am einfachsten einhalten?', answer: 'Ein fester Wocheneinkaufszettel und ein separates Unterkonto oder Bargeldumschlag für Lebensmittel schaffen klare Kontrolle.' },
    ],
...

  {
    ...
    category: 'haushalt-energie',
    subcategory: 'Heizung & Gas',
    metaTitle: 'Energieeffizienzklasse Rechner Gebäude – Endenergiebedarf A+ bis H GEG',
    ...
    faqs: [
      { question: 'Ist die Angabe der Energieklasse im Immobilieninserat Pflicht?', answer: 'Ja, nach § 87 GEG müssen Energieeffizienzklasse und Energiekennwert bei Verkauf oder Vermietung zwingend in der Anzeige genannt werden.' },
      { question: 'Was ist der Unterschied zwischen Bedarfs- und Verbrauchsausweis?', answer: 'Der Bedarfsausweis basiert auf einer rechnerischen Analyse der Bausubstanz; der Verbrauchsausweis basiert auf den realen Heizkostenabrechnungen der letzten 3 Jahre.' },
    ],
...
console.log('Successfully wrote extra wohnenHaushalt.ts with 42 calculators!');
