...
const calcs = [
  {
    ...
    name: 'Alkohol Verkochen Rechner (Restalkohol in Saucen & Schmorgerichten)',
    ...
    inputs: [
      { id: 'alcoholMl', label: 'Zugegebene Alkoholmenge (z. B. Rotwein, Weißwein, Bier)', type: 'number', defaultValue: 250, min: 10, max: 2000, step: 25, unit: 'ml' },
      { id: 'volPercent', label: 'Alkoholgehalt des Getränks', type: 'number', defaultValue: 13.0, min: 1.0, max: 80.0, step: 0.5, unit: 'Vol.-%' },
      {
        id: 'cookingMethod',
        ...
        options: [
          { value: 'flambee', label: 'Flambieren (kurz angezündet – ca. 75 % Restalkohol)' },
          { value: 'stir15', label: 'Kurz aufgekocht / 15 Min. köcheln (ca. 40 % Restalkohol)' },
          { value: 'simmer30', label: '30 Minuten leicht köcheln (ca. 35 % Restalkohol)' },
          { value: 'simmer60', label: '1 Stunde köcheln (ca. 25 % Restalkohol)' },
          { value: 'simmer120', label: '2 Stunden Schmorgericht (ca. 10 % Restalkohol)' },
          { value: 'simmer150', label: 'Über 2,5 Stunden geschmort (ca. 5 % Restalkohol)' },
        ],
      },
      { id: 'portionsCount', label: 'Anzahl Portionen des Gerichts', type: 'number', defaultValue: 4, min: 1, max: 20, step: 1, unit: 'Portionen' },
    ],
    calculateCode: `const ml = Number(inputs.alcoholMl) || 0;
...
return {
  ...
  summaryText: 'Von ursprünglich ' + formatNumber(pureAlcoholGramsInitial, 1) + ' g Alkohol verbleiben nach dem Garen noch rund ' + formatNumber(remainingAlcoholGrams, 1) + ' g (' + (retentionRate * 100) + ' %) im Essen – das sind ca. ' + formatNumber(remainingPerPortionG, 2) + ' g pro Portion.',
    ...
    workedExample: {
      ...
      inputValues: [{ label: 'Wein', value: '250 ml' }, { label: 'Alkohol', value: '13 Vol.-%' }, { label: 'Kochzeit', value: '2 Stunden (10 % Rest)' }],
      steps: ['Ausgangsalkohol = 250 × 0,13 × 0,789 = 25,6 g reines Ethanol', 'Rest nach 2h = 25,6 g × 0,10 = 2,56 g gesamt', 'Pro Portion (bei 4 Portionen) = 2,56 / 4 = 0,64 g Alkohol'],
      result: '0,64 g reiner Alkohol pro Portion',
    },
    faqs: [
      { question: 'Verkocht Alkohol beim Flambieren?', answer: 'Nein, beim kurzen Flambieren verbrennt fast nur der Alkoholdampf an der Oberfläche. Rund 75 % des gesamten Alkohols verbleiben in der Speise.' },
      { question: 'Dürfen Kinder Gerichte essen, die mit Wein gekocht wurden?', answer: 'Aus toxikologischer Sicht ist die Dosis nach langem Schmoren gering, Suchtexperten und Kinderärzte raten jedoch ab, um Kinder nicht frühzeitig an den typischen Weingeschmack zu gewöhnen.' },
    ],
...

  {
    ...
    inputs: [
      { id: 'eggsCount', label: 'Anzahl zu ersetzender Eier (Größe M)', type: 'number', defaultValue: 2, min: 1, max: 12, step: 1, unit: 'Eier' },
      {
        id: 'subType',
        ...
        options: [
          { value: 'applesauce', label: 'Apfelmark / Apfelmus (saftig für Rührkuchen – 60 bis 80 g je Ei)' },
          { value: 'banana', label: 'Reife Banane püriert (für süße Kuchen/Pancakes – 1/2 Banane je Ei)' },
          { value: 'flaxseed', label: 'Leinsamen-Ei / Chia-Ei (1 EL gemahlen + 3 EL Wasser je Ei)' },
          { value: 'silkenTofu', label: 'Seidentofu püriert (neutral für Quiches/Kuchen – 60 g je Ei)' },
          { value: 'aquafaba', label: 'Aquafaba / Kichererbsenwasser (für Eischnee/Baiser – 3 EL je Eiweiß)' },
        ],
      },
    ],
    calculateCode: `const count = Number(inputs.eggsCount) || 1;
...
return {
  ...
  summaryText: 'Um ' + count + ' Ei(er) zu ersetzen, benötigen Sie ' + amountStr + '. ' + tip,
    ...
    workedExample: {
      ...
      inputValues: [{ label: 'Eier', value: '2 Eier (Größe M)' }, { label: 'Ersatz', value: 'Apfelmark' }],
      steps: ['2 Eier × 70 g Apfelmark = 140 g Apfelmark', 'Teig wie gewohnt mit 1 TL extra Backpulver anrühren'],
      result: '140 g Apfelmark (ca. 4-5 gehäufte Esslöffel)',
    },
    faqs: [
      { question: 'Was ist Aquafaba?', answer: 'Aquafaba ist das dickflüssige Kochwasser von Kichererbsen (oder Bohnen aus der Dose). Aufgrund seiner Eiweiß- und Stärkestruktur lässt es sich wie echter Eischnee steif schlagen.' },
      { question: 'Kann man mit Bananen-Ei auch Biskuit backen?', answer: 'Nein, für lockeren Biskuit eignen sich schwere Fruchtpürees nicht. Hier nutzt man besser aufgeschlagenes Aquafaba oder spezielles Ei-Ersatzpulver auf Lupinen-/Stärkebasis.' },
    ],
...

  {
    ...
    name: 'Reis kochen Rechner (Wasser-Reis-Verhältnis & Garzeit nach Reissorte)',
    shortName: 'Reis kochen Rechner',
    category: 'kochen-backen',
    subcategory: 'Kochen & Garen',
    ...
    inputs: [
      { id: 'riceGrams', label: 'Reismenge (ungekocht)', type: 'number', defaultValue: 200, min: 50, max: 2000, step: 25, unit: 'g Reis' },
      {
        id: 'riceType',
        label: 'Reissorte',
        type: 'select',
        ...
        options: [
          { value: 'basmati', label: 'Basmati-Reis (Verhältnis 1 : 1,5 – ca. 12-15 Min.)' },
          { value: 'jasmine', label: 'Jasmin-Reis / Duftreis (Verhältnis 1 : 1,3 – ca. 12-14 Min.)' },
          { value: 'wholegrain', label: 'Vollkornreis / Naturreis (Verhältnis 1 : 2,0 – ca. 35-45 Min.)' },
          { value: 'sushi', label: 'Sushireis / Rundkorn (Verhältnis 1 : 1,2 – ca. 15 Min. + Dämpfen)' },
          { value: 'risotto', label: 'Risotto-Reis (Arborio/Carnaroli – Verhältnis 1 : 3,0 Brühe)' },
        ],
      },
    ],
    calculateCode: `const riceG = Number(inputs.riceGrams) || 200;
...
return {
  ...
  summaryText: 'Für ' + riceG + ' g Reis benötigen Sie genau ' + waterMl + ' ml Wasser und ca. ' + saltG + ' g Salz. Bei geschlossenem Deckel auf kleinster Stufe ca. ' + cookMins + ' Minuten sanft köcheln lassen.',
    ...
    workedExample: {
      ...
      inputValues: [{ label: 'Reis', value: '200 g' }, { label: 'Sorte', value: 'Basmati (1:1,5)' }],
      steps: ['Wasser = 200 g × 1,5 = 300 ml Wasser', '1 Mal aufkochen, dann Deckel drauf und 12-14 Min. auf kleinster Stufe ziehen lassen'],
      result: '300 ml Wasser (ergibt ca. 500 g lockeren Reis)',
    },
    faqs: [
      { question: 'Warum sollte man Reis vor dem Kochen waschen?', answer: 'Durch das Waschen wird überschüssige Oberflächenstärke entfernt. Dadurch klebt der Reis nach dem Kochen nicht klumpig zusammen, sondern bleibt wunderbar körnig und locker.' },
      { question: 'Darf man beim Reis kochen den Deckel anheben?', answer: 'Nein! Wenn der Deckel vor Ablauf der Garzeit geöffnet wird, entweicht der notwendige heiße Dampf und der Reis gart ungleichmäßig oder brennt am Boden an.' },
    ],
...

  {
    ...
    name: 'Nudeln Rechner (Trocken- vs. Gekocht-Gewicht & Portionsgröße)',
    shortName: 'Nudeln Rechner',
    category: 'kochen-backen',
    subcategory: 'Kochen & Garen',
    ...
    inputs: [
      { id: 'personsCount', label: 'Anzahl Personen', type: 'number', defaultValue: 4, min: 1, max: 50, step: 1, unit: 'Personen' },
      {
        id: 'mealType',
        ...
        options: [
          { value: 'mainDish', label: 'Hauptgericht / normaler Hunger (ca. 100 bis 125 g p.P.)' },
          { value: 'bigHunger', label: 'Großer Hunger / Sportler (ca. 150 g p.P.)' },
          { value: 'sideDish', label: 'Beilage / Vorspeise (ca. 60 bis 80 g p.P.)' },
          { value: 'kids', label: 'Kinderportion (ca. 50 bis 60 g p.P.)' },
        ],
      },
    ],
    calculateCode: `const persons = Number(inputs.personsCount) || 1;
...
return {
  ...
  summaryText: 'Für ' + persons + ' Personen benötigen Sie ' + totalDryG + ' g trockene Nudeln (' + dryPerPersonG + ' g p.P.). Beim Kochen saugen sie Wasser auf und ergeben ca. ' + formatNumber(cookedWeightG, 0) + ' g fertige Pasta.',
    ...
    workedExample: {
      ...
      inputValues: [{ label: 'Personen', value: '4' }, { label: 'Hunger', value: 'Normal (110 g)' }],
      steps: ['Trockengewicht = 4 × 110 g = 440 g (fast 1 Paket à 500 g)', 'Gekochtes Gewicht = 440 × 2,28 ≈ 1.000 g Pasta', 'Wasser = 4 bis 5 Liter Wasser mit 40 g Salz'],
      result: '440 g Trockennudeln (ca. 1 kg gekocht)',
    },
    faqs: [
      { question: 'Sollte man Öl ins Nudelwasser geben?', answer: 'Nein! Öl schwimmt nur auf dem Wasser und legt sich beim Abgießen wie ein Film um die Nudeln. Dadurch kann die Nudeloberfläche die Sauce später viel schlechter aufnehmen.' },
      { question: 'Sollte man Nudeln nach dem Kochen mit kaltem Wasser abschrecken?', answer: 'Nein, niemals abschrecken (außer bei kaltem Nudelsalat)! Das Abschrecken spült die wertvolle Oberflächenstärke ab, die für die Saucenbindung unverzichtbar ist.' },
    ],
...

  {
    ...
    inputs: [
      { id: 'desiredSourdoughG', label: 'Benötigte Sauerteigmenge laut Rezept', type: 'number', defaultValue: 200, min: 20, max: 2000, step: 10, unit: 'g Sauerteig' },
      {
        id: 'feedingRatio',
        ...
        defaultValue: '1_1_1',
        options: [
          { value: '1_1_1', label: '1 : 1 : 1 (Klassisch – schnelle Reife in ca. 4-6 Stunden)' },
          { value: '1_2_2', label: '1 : 2 : 2 (Mittlere Reife in ca. 6-8 Stunden)' },
          { value: '1_5_5', label: '1 : 5 : 5 (Sehr milde Führung über Nacht / ca. 10-12 Stunden)' },
        ],
      },
      { id: 'reserveG', label: 'Zusätzliche Reserve für nächstes Anstellgut', type: 'number', defaultValue: 30, min: 10, max: 100, step: 5, unit: 'g Reserve' },
    ],
    calculateCode: `const targetG = Number(inputs.desiredSourdoughG) || 200;
...
return {
  ...
  summaryText: 'Mischen Sie ' + starterG + ' g Anstellgut mit ' + flourG + ' g Mehl und ' + waterG + ' g handwarmem Wasser. Ergibt ' + actualTotal + ' g reifen Sauerteig (' + targetG + ' g fürs Rezept, ' + reserveG + ' g zurück ins Glas als Reserve).',
    ...
    workedExample: {
      ...
      inputValues: [{ label: 'Zielmenge', value: '230 g gesamt' }, { label: 'Verhältnis', value: '1:1:1 (3 Teile)' }],
      steps: ['1 Teil = 230 g / 3 ≈ 77 g', '77 g Anstellgut + 77 g Mehl + 77 g Wasser = 231 g Sauerteig'],
      result: 'je 77 g Anstellgut, Mehl und Wasser',
    },
    faqs: [
      { question: 'Wann ist der Sauerteig backbereit (Peak)?', answer: 'Der Sauerteig ist backbereit, wenn er sein Volumen verdoppelt bis verdreifacht hat, eine kuppelförmige Oberfläche mit vielen Bläschen zeigt und kurz vor dem Wiederabsinken steht.' },
      { question: 'Wie übersteht Anstellgut den Urlaub?', answer: 'Gut gefüttert hält sich Anstellgut im Schraubglas im Kühlschrank problemlos 2 bis 3 Wochen. Für längere Pausen kann man ihn dünn auf Backpapier verstreichen, trocknen und als Trockensauerteig jahrelang aufbewahren.' },
    ],
...

  {
    ...
    name: 'Marmelade & Gelierzucker Rechner (1:1, 2:1 & 3:1 Zucker)',
    ...
    inputs: [
      { id: 'fruitGrams', label: 'Vorbereitete Früchte (geputzt & entsteint)', type: 'number', defaultValue: 1000, min: 100, max: 20000, step: 50, unit: 'g Früchte' },
      {
        id: 'sugarType',
        ...
        defaultValue: '2_1',
        options: [
          { value: '2_1', label: 'Gelierzucker 2:1 (Fruchtig-süß – 500 g Zucker auf 1.000 g Frucht – Empfehlung)' },
          { value: '3_1', label: 'Gelierzucker 3:1 (Sehr fruchtig, weniger süß – 333 g Zucker auf 1.000 g Frucht)' },
          { value: '1_1', label: 'Gelierzucker 1:1 (Klassisch süß, extrem lange haltbar – 1.000 g Zucker auf 1.000 g Frucht)' },
        ],
      },
      { id: 'jarSizeMl', label: 'Fassungsvermögen der Marmeladengläser', type: 'select', defaultValue: '250', options: [{ value: '200', label: 'Kleine Gläser (200 ml)' }, { value: '250', label: 'Standardgläser (250 ml)' }, { value: '350', label: 'Große Gläser (350 ml)' }] },
    ],
    calculateCode: `const fruitG = Number(inputs.fruitGrams) || 0;
...
return {
  primary: { id: 'sugarAmount', label: 'Benötigter Gelierzucker (' + (sugarType.replace('_', ':')) + ')', value: sugarG, formattedValue: sugarG + ' g Gelierzucker', highlight: true },
  secondary: [
  ...
  summaryText: 'Für ' + fruitG + ' g Früchte benötigen Sie ' + sugarG + ' g Gelierzucker ' + (sugarType.replace('_', ':')) + ' und ca. ' + lemonJuiceMl + ' ml Zitronensaft. Ergibt rund ' + jarsCount + ' Gläser à ' + jarMl + ' ml.',
};`,
    ...
    workedExample: {
      ...
      inputValues: [{ label: 'Früchte', value: '1.500 g' }, { label: 'Zucker', value: '2:1' }],
      steps: ['Gelierzucker = 1.500 / 2 = 750 g Zucker', 'Marmelade ca. 2.070 g -> ca. 7 Gläser à 250 ml'],
      result: '750 g Gelierzucker (7 Gläser)',
    },
    faqs: [
      { question: 'Wie funktioniert die Gelierprobe richtig?', answer: 'Geben Sie nach 4 Minuten sprudelndem Kochen 1 Teelöffel heiße Marmelade auf einen eiskalten Teller aus dem Tiefkühlfach. Wird die Masse nach 1 Minute fest, ist die Marmelade fertig!' },
      { question: 'Muss man heiße Marmeladengläser auf den Kopf stellen?', answer: 'Nein, nach heutigem Stand der Hygiene ist das nicht nötig und kann sogar Schadstoffe aus dem Deckelgummi lösen. Gründlich sterilisierte Gläser und heißes Einfüllen genügen völlig für ein Vakuum.' },
    ],
...

  {
    ...
    name: 'Kaffee Rechner (Golden Cup Ratio Kaffeemehl in g nach Tassenanzahl)',
    ...
    inputs: [
      { id: 'cupsCount', label: 'Anzahl Kaffeetassen', type: 'number', defaultValue: 4, min: 1, max: 30, step: 1, unit: 'Tassen' },
      {
        id: 'cupSizeMl',
        label: 'Tassengröße',
        type: 'select',
        defaultValue: '150',
        options: [
          { value: '125', label: 'Kleine Kaffeetasse (125 ml – Kaffeemaschinen-Norm)' },
          { value: '150', label: 'Standard Kaffeetasse (150 ml)' },
          { value: '250', label: 'Großer Kaffeebecher / Mug (250 ml)' },
        ],
      },
      {
        id: 'strength',
        ...
        options: [
          { value: 'mild', label: 'Mild (ca. 55 g Kaffee / 1.000 ml Wasser – Verhältnis 1:18)' },
          { value: 'standard', label: 'Ausgewogen / SCA Golden Cup (60 g / 1.000 ml – Verhältnis 1:16,7)' },
          { value: 'strong', label: 'Kräftig (ca. 65 g Kaffee / 1.000 ml – Verhältnis 1:15)' },
        ],
      },
    ],
    calculateCode: `const cups = Number(inputs.cupsCount) || 1;
...
return {
  ...
  summaryText: 'Für ' + cups + ' Tassen Kaffee (' + totalWaterMl + ' ml Wasser) benötigen Sie exakt ' + formatNumber(coffeeGrams, 1) + ' g frisch gemahlenes Kaffeemehl (ca. ' + spoonsCount + ' Kaffeelote).',
    ...
    workedExample: {
      ...
      inputValues: [{ label: 'Tassen', value: '4 (600 ml)' }, { label: 'Stärke', value: 'Golden Cup (60 g/l)' }],
      steps: ['Kaffee = 0,6 Liter × 60 g/l = 36 g Kaffeemehl', 'Entspricht ca. 5 Kaffeeloten'],
      result: '36,0 g Kaffeemehl',
    },
    faqs: [
      { question: 'Warum sollte Kaffeewasser nicht kochend heiß sein?', answer: 'Kochendes Wasser (100°C) verbrennt die feinen Kaffeearomen und löst übermäßig viele Bitterstoffe und Gerbsäuren. Warten Sie nach dem Aufkochen ca. 45 bis 60 Sekunden, bis das Wasser 92-96°C hat.' },
      { question: 'Wie fein sollte der Kaffee für die French Press gemahlen werden?', answer: 'Für die French Press benötigt man einen groben Mahlgrad (wie Meersalz), damit das Kaffeepulver nicht durch das Metallsieb rutscht und der Kaffee nicht überextrahiert.' },
    ],
...

  {
    ...
    inputs: [
      { id: 'spiritMl', label: 'Basis-Spirituose (z. B. Gin, Wodka, Rum 40 Vol.-%)', type: 'number', defaultValue: 50, min: 0, max: 250, step: 5, unit: 'ml' },
      { id: 'spiritVolPct', label: 'Alkoholgehalt Spirituose', type: 'number', defaultValue: 40.0, min: 15.0, max: 80.0, step: 0.5, unit: 'Vol.-%' },
      { id: 'liqueurMl', label: 'Likör / Wermut (z. B. Aperol, Triple Sec 15-20 Vol.-%)', type: 'number', defaultValue: 0, min: 0, max: 150, step: 5, unit: 'ml' },
      { id: 'liqueurVolPct', label: 'Alkoholgehalt Likör', type: 'number', defaultValue: 20.0, min: 10.0, max: 50.0, step: 0.5, unit: 'Vol.-%' },
      { id: 'fillerMl', label: 'Alkoholfreier Filler (Tonic, Cola, Saft, Soda)', type: 'number', defaultValue: 150, min: 0, max: 500, step: 10, unit: 'ml' },
      { id: 'iceMeltMl', label: 'Schmelzwasser durch Eiswürfel ca.', type: 'number', defaultValue: 30, min: 0, max: 100, step: 5, unit: 'ml' },
    ],
    calculateCode: `const sMl = Number(inputs.spiritMl) || 0;
...
return {
  ...
  summaryText: 'Ihr Cocktail (Gesamtvolumen ' + totalDrinkVolumeMl + ' ml) hat einen Alkoholgehalt von ca. ' + formatNumber(volPercentFinal, 1) + ' Vol.-% und enthält ' + formatNumber(pureGrams, 1) + ' g reinen Alkohol (entspricht ca. ' + formatNumber(beerBottles, 1) + ' Flaschen Bier).',
    ...
    workedExample: {
      ...
      inputValues: [{ label: 'Gin', value: '50 ml (40 %)' }, { label: 'Tonic', value: '150 ml' }, { label: 'Eis', value: '30 ml Schmelzwasser' }],
      steps: ['Ethanol = 50 × 0,40 = 20 ml reines Ethanol', 'Gesamtvolumen = 50 + 150 + 30 = 230 ml', 'Vol.-% = (20 / 230) × 100 = 8,7 Vol.-%'],
      result: '8,7 Vol.-% (15,8 g reiner Alkohol)',
    },
    faqs: [
      { question: 'Wie viel Schmelzwasser entsteht beim Shaken eines Cocktails?', answer: 'Beim kräftigen Shaken mit Eis schmelzen pro Drink etwa 25 bis 35 ml Wasser. Profi-Barkeeper berechnen diese Verdünnung fest in ihre Rezepturen ein.' },
      { question: 'Wie viel reiner Alkohol gilt für Erwachsene als risikoarm?', answer: 'Nach den Leitlinien der DGE sollten gesunde Frauen maximal 10 bis 12 g Alkohol pro Tag (ca. 1 kleines Bier oder 1 kleines Glas Wein) und gesunde Männer maximal 20 bis 24 g Alkohol pro Tag trinken, mit mindestens zwei alkoholfreien Tagen pro Woche.' },
    ],
    relatedSlugs: ['alkohol-verkochungs-rechner', 'promillerechner', 'portionsrechner'],
  },
];

console.log('Building kochen part 2 with', calcs.length, 'calculators');
fs.writeFileSync(path.join(__dirname, 'calcs-kochen-part2.json'), JSON.stringify(calcs, null, 2), 'utf8');
console.log('Saved calcs-kochen-part2.json');
