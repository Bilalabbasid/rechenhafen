...
const calculators = [
  // 1. Mittelwert, Median, Modus
  {
    ...
    inputs: [
      { id: 'val1', label: 'Wert 1', type: 'number', defaultValue: 10, step: 0.1 },
      { id: 'val2', label: 'Wert 2', type: 'number', defaultValue: 15, step: 0.1 },
      { id: 'val3', label: 'Wert 3', type: 'number', defaultValue: 20, step: 0.1 },
      { id: 'val4', label: 'Wert 4', type: 'number', defaultValue: 20, step: 0.1 },
      { id: 'val5', label: 'Wert 5', type: 'number', defaultValue: 35, step: 0.1 },
    ],
    calculate: `(inputs) => {
    ...
    workedExample: {
      ...
      inputs: { val1: 10, val2: 15, val3: 20, val4: 20, val5: 35 },
    ...
    faqs: [
      { question: 'Wann sollte man den Median statt des Mittelwerts nutzen?', answer: 'Bei schiefen Verteilungen mit extremen Ausreißern (z.B. Einkommen oder Vermögen) spiegelt der Median den typischen Wert viel realistischer wider als der Mittelwert.' },
    ],
    relatedSlugs: ['standardabweichung-rechner', 'notendurchschnitt-rechner', 'variationskoeffizient-rechner'],
  },

  // 2. Varianz & Standardabweichung (Stichprobe vs Population)
  {
    id: 'varianz-standardabweichung-stichprobe-rechner',
    slug: 'varianz-standardabweichung-stichprobe-rechner',
    name: 'Stichprobenvarianz & Populationsvarianz Rechner',
    ...
    inputs: [
      { id: 'v1', label: 'Messwert 1', type: 'number', defaultValue: 12, step: 0.1 },
      { id: 'v2', label: 'Messwert 2', type: 'number', defaultValue: 15, step: 0.1 },
      { id: 'v3', label: 'Messwert 3', type: 'number', defaultValue: 18, step: 0.1 },
      { id: 'v4', label: 'Messwert 4', type: 'number', defaultValue: 21, step: 0.1 },
      { id: 'v5', label: 'Messwert 5', type: 'number', defaultValue: 24, step: 0.1 },
    ],
    calculate: `(inputs) => {
      const arr = [
        Number(inputs.v1) || 0,
    ...
    workedExample: {
      ...
      inputs: { v1: 12, v2: 15, v3: 18, v4: 21, v5: 24 },
    ...
    faqs: [
      { question: 'Warum teilt man durch n - 1 statt n?', answer: 'Weil die Abweichungen vom Stichprobenmittelwert x̄ berechnet werden, der selbst aus den Daten geschätzt wurde. Dies kostet einen Freiheitsgrad (df = n - 1).' },
    ],
    relatedSlugs: ['standardabweichung-rechner', 'mittelwert-median-modus-rechner', 'konfidenzintervall-rechner'],
  },

  // 3. Korrelationskoeffizient Rechner (Pearson r)
  {
    ...
    inputs: [
      { id: 'x1', label: 'X1', type: 'number', defaultValue: 1, step: 0.1 },
      { id: 'y1', label: 'Y1', type: 'number', defaultValue: 2, step: 0.1 },
      { id: 'x2', label: 'X2', type: 'number', defaultValue: 2, step: 0.1 },
      { id: 'y2', label: 'Y2', type: 'number', defaultValue: 3, step: 0.1 },
      { id: 'x3', label: 'X3', type: 'number', defaultValue: 3, step: 0.1 },
      { id: 'y3', label: 'Y3', type: 'number', defaultValue: 5, step: 0.1 },
      { id: 'x4', label: 'X4', type: 'number', defaultValue: 4, step: 0.1 },
      { id: 'y4', label: 'Y4', type: 'number', defaultValue: 7, step: 0.1 },
      { id: 'x5', label: 'X5', type: 'number', defaultValue: 5, step: 0.1 },
      { id: 'y5', label: 'Y5', type: 'number', defaultValue: 8, step: 0.1 },
    ],
    calculate: `(inputs) => {
    ...
    workedExample: {
      ...
      inputs: { x1: 1, y1: 2, x2: 2, y2: 3, x3: 3, y3: 5, x4: 4, y4: 7, x5: 5, y5: 8 },
    ...
    faqs: [
      { question: 'Was bedeutet das Bestimmtheitsmaß R²?', answer: 'R² gibt den Anteil der Varianz in Y an, der durch die lineare Beziehung zu X erklärt werden kann.' },
    ],
  ...
  // 4. Z-Score Rechner
  {
    ...
    inputs: [
      { id: 'x', label: 'Messwert (X)', type: 'number', defaultValue: 115, step: 0.1 },
      { id: 'mean', label: 'Mittelwert (μ)', type: 'number', defaultValue: 100, step: 0.1 },
      { id: 'sd', label: 'Standardabweichung (σ)', type: 'number', defaultValue: 15, step: 0.1, min: 0.0001 },
    ],
    calculate: `(inputs) => {
    ...
    faqs: [
      { question: 'Was bedeutet ein negativer Z-Score?', answer: 'Ein negativer Z-Wert bedeutet, dass der Beobachtungswert unterhalb des Mittelwerts liegt.' },
    ],
  ...
  // 5. p-Wert Hypothesentest Rechner
  {
    ...
    name: 'p-Wert Rechner (Z-Test Hypothesentest)',
    ...
    inputs: [
      { id: 'zStat', label: 'Z-Prüfgröße (Teststatistik)', type: 'number', defaultValue: 1.96, step: 0.01 },
      { id: 'alpha', label: 'Signifikanzniveau α (%)', type: 'number', defaultValue: 5, step: 1, min: 0.1, max: 20 },
    ],
    calculate: `(inputs) => {
    ...
    faqs: [
      { question: 'Beweist ein niedriger p-Wert, dass die Forschungshypothese wahr ist?', answer: 'Nein, er zeigt lediglich, dass die beobachteten Daten unter der Annahme des Zufalls (H0) sehr unwahrscheinlich sind.' },
    ],
    relatedSlugs: ['z-score-normalverteilung-rechner', 't-test-rechner', 'konfidenzintervall-rechner'],
  },

  // 6. t-Test Rechner (Einstichproben)
  {
    id: 't-test-rechner',
    slug: 't-test-rechner',
    ...
    inputs: [
      { id: 'sampleMean', label: 'Stichprobenmittelwert (x̄)', type: 'number', defaultValue: 104, step: 0.1 },
      { id: 'hypoMean', label: 'Referenzwert (μ0)', type: 'number', defaultValue: 100, step: 0.1 },
      { id: 'sampleSd', label: 'Stichproben-Streuung (s)', type: 'number', defaultValue: 8, step: 0.1, min: 0.001 },
      { id: 'sampleN', label: 'Stichprobenumfang (n)', type: 'number', defaultValue: 25, step: 1, min: 2 },
    ],
    calculate: `(inputs) => {
    ...
    workedExample: {
      ...
      inputs: { sampleMean: 104, hypoMean: 100, sampleSd: 8, sampleN: 25 },
    ...
    faqs: [
      { question: 'Was ist der Standardfehler (Standard Error)?', answer: 'Der Standardfehler misst die Streuung des Stichprobenmittelwerts um den wahren Populationsmittelwert.' },
    ],
  ...
  // 7. Konfidenzintervall Rechner
  {
    ...
    inputs: [
      { id: 'mean', label: 'Mittelwert (x̄)', type: 'number', defaultValue: 50, step: 0.1 },
      { id: 'sd', label: 'Standardabweichung (s)', type: 'number', defaultValue: 10, step: 0.1, min: 0.001 },
      { id: 'n', label: 'Stichprobengröße (n)', type: 'number', defaultValue: 100, step: 1, min: 2 },
      {
        id: 'confLevel',
    ...
    workedExample: {
      ...
      inputs: { mean: 50, sd: 10, n: 100, confLevel: '95' },
    ...
    faqs: [
      { question: 'Bedeutet 95 % Konfidenz, dass der wahre Wert mit 95 % im berechneten Intervall liegt?', answer: 'Klassisch-frequentistisch bedeutet es: Würde man 100 Stichproben ziehen und je ein Intervall berechnen, würden im Schnitt 95 dieser Intervalle den wahren festen Wert enthalten.' },
    ],
    relatedSlugs: ['stichprobengroesse-rechner', 'p-wert-hypothesentest-rechner', 'standardabweichung-rechner'],
  },

  // 8. Stichprobengröße Rechner
  {
    id: 'stichprobengroesse-rechner',
    slug: 'stichprobengroesse-rechner',
    name: 'Stichprobengröße Rechner (Umfragen & Studien)',
    ...
    inputs: [
      { id: 'popSize', label: 'Grundgesamtheit (0 = unendlich groß)', type: 'number', defaultValue: 10000, step: 1000, min: 0 },
      { id: 'marginError', label: 'Gewünschte Fehlerspanne (± %)', type: 'number', defaultValue: 3, step: 0.5, min: 0.5, max: 20 },
      {
        id: 'confLevel',
        label: 'Konfidenzniveau',
        type: 'select',
    ...
    workedExample: {
      ...
      inputs: { popSize: 10000, marginError: 3, confLevel: '95' },
    ...
    faqs: [
      { question: 'Muss die Stichprobe bei größeren Populationen proportional größer sein?', answer: 'Nein! Ab einer Populationsgröße von etwa 20.000 Personen bleibt die benötigte Stichprobengröße nahezu konstant.' },
    ],
  ...
  // 9. Quartile & Box-Plot Rechner
  {
    ...
    inputs: [
      { id: 'w1', label: 'Wert 1', type: 'number', defaultValue: 12, step: 0.1 },
      { id: 'w2', label: 'Wert 2', type: 'number', defaultValue: 15, step: 0.1 },
      { id: 'w3', label: 'Wert 3', type: 'number', defaultValue: 18, step: 0.1 },
      { id: 'w4', label: 'Wert 4', type: 'number', defaultValue: 24, step: 0.1 },
      { id: 'w5', label: 'Wert 5', type: 'number', defaultValue: 30, step: 0.1 },
      { id: 'w6', label: 'Wert 6', type: 'number', defaultValue: 35, step: 0.1 },
      { id: 'w7', label: 'Wert 7', type: 'number', defaultValue: 42, step: 0.1 },
    ],
    calculate: `(inputs) => {
      const arr = [
        Number(inputs.w1) || 0,
    ...
    workedExample: {
      ...
      inputs: { w1: 12, w2: 15, w3: 18, w4: 24, w5: 30, w6: 35, w7: 42 },
    ...
    faqs: [
      { question: 'Warum ist der IQR oft aussagekräftiger als die Standardabweichung?', answer: 'Weil der IQR unempfindlich gegenüber extremen Ausreißern ist, da er nur die mittleren 50 % der Daten betrachtet.' },
    ],
  ...
  // 10. Binomialverteilung Rechner
  {
    ...
    name: 'Binomialverteilung Rechner (Bernoulli-Kette)',
    ...
    inputs: [
      { id: 'trials', label: 'Anzahl der Versuche (n)', type: 'number', defaultValue: 10, step: 1, min: 1, max: 100 },
      { id: 'prob', label: 'Trefferwahrscheinlichkeit p (%)', type: 'number', defaultValue: 20, step: 1, min: 0.1, max: 99.9 },
      { id: 'successes', label: 'Gewünschte Treffer (k)', type: 'number', defaultValue: 2, step: 1, min: 0, max: 100 },
    ],
    calculate: `(inputs) => {
    ...
    workedExample: {
      ...
      inputs: { trials: 10, prob: 20, successes: 2 },
    ...
    faqs: [
      { question: 'Wann kann die Binomialverteilung durch die Normalverteilung angenähert werden?', answer: 'Wenn die Laplace-Bedingung σ = √(n · p · (1 - p)) > 3 erfüllt ist.' },
    ],
  ...
  // 11. Poisson-Verteilung Rechner
  {
    ...
    name: 'Poisson-Verteilung Rechner (Ereignisrate λ)',
    ...
    inputs: [
      { id: 'lambdaRate', label: 'Durchschnittliche Ereignisrate (λ)', type: 'number', defaultValue: 4, step: 0.1, min: 0.01 },
      { id: 'kEvents', label: 'Beobachtete Ereignisse (k)', type: 'number', defaultValue: 4, step: 1, min: 0 },
    ],
    calculate: `(inputs) => {
    ...
    faqs: [
      { question: 'Wann wird die Poisson-Verteilung statt der Binomialverteilung verwendet?', answer: 'Wenn n sehr groß und p sehr klein ist (Faustregel: n ≥ 50 und p ≤ 0,05), nähert die Poisson-Verteilung die Binomialverteilung hervorragend an.' },
    ],
    relatedSlugs: ['binomialverteilung-rechner', 'kombinatorik-n-ueber-k-rechner', 'wahrscheinlichkeit-wuerfel-muenze-rechner'],
  },

  // 12. Kombinatorik (n über k) Rechner
  {
    ...
    inputs: [
      { id: 'totalN', label: 'Gesamtzahl Elemente (n)', type: 'number', defaultValue: 49, step: 1, min: 1, max: 100 },
      { id: 'chooseK', label: 'Auswahl Elemente (k)', type: 'number', defaultValue: 6, step: 1, min: 1, max: 100 },
    ],
    calculate: `(inputs) => {
    ...
    faqs: [
      { question: 'Was ist der Unterschied zwischen Permutation und Kombination?', answer: 'Bei einer Permutation kommt es auf die genaue Reihenfolge an (wie bei einem Zahlenschloss). Bei einer Kombination ist die Reihenfolge egal (wie bei den gezogenen Lottozahlen).' },
    ],
  ...
  // 13. Lineare Regression Rechner
  {
    ...
    name: 'Lineare Regression Rechner (y = ax + b & R²)',
    ...
    inputs: [
      { id: 'x1', label: 'X1', type: 'number', defaultValue: 1, step: 0.1 },
      { id: 'y1', label: 'Y1', type: 'number', defaultValue: 2.5, step: 0.1 },
      { id: 'x2', label: 'X2', type: 'number', defaultValue: 2, step: 0.1 },
      { id: 'y2', label: 'Y2', type: 'number', defaultValue: 4.8, step: 0.1 },
      { id: 'x3', label: 'X3', type: 'number', defaultValue: 3, step: 0.1 },
      { id: 'y3', label: 'Y3', type: 'number', defaultValue: 6.9, step: 0.1 },
      { id: 'x4', label: 'X4', type: 'number', defaultValue: 4, step: 0.1 },
      { id: 'y4', label: 'Y4', type: 'number', defaultValue: 9.1, step: 0.1 },
    ],
    calculate: `(inputs) => {
    ...
    workedExample: {
      ...
      inputs: { x1: 1, y1: 2.5, x2: 2, y2: 4.8, x3: 3, y3: 6.9, x4: 4, y4: 9.1 },
    ...
    faqs: [
      { question: 'Darf man die Gerade zur Vorhersage beliebiger Werte nutzen?', answer: 'Nur innerhalb des gemessenen Wertebereichs (Interpolation). Eine Extrapolation weit außerhalb birgt hohe Risiken von Fehlprognosen.' },
    ],
  ...
  // 14. Kovarianz Rechner
  {
    ...
    inputs: [
      { id: 'x1', label: 'X1', type: 'number', defaultValue: 10, step: 0.1 },
      { id: 'y1', label: 'Y1', type: 'number', defaultValue: 12, step: 0.1 },
      { id: 'x2', label: 'X2', type: 'number', defaultValue: 20, step: 0.1 },
      { id: 'y2', label: 'Y2', type: 'number', defaultValue: 22, step: 0.1 },
      { id: 'x3', label: 'X3', type: 'number', defaultValue: 30, step: 0.1 },
      { id: 'y3', label: 'Y3', type: 'number', defaultValue: 35, step: 0.1 },
      { id: 'x4', label: 'X4', type: 'number', defaultValue: 40, step: 0.1 },
      { id: 'y4', label: 'Y4', type: 'number', defaultValue: 41, step: 0.1 },
    ],
    calculate: `(inputs) => {
      const xs = [Number(inputs.x1)||0, Number(inputs.x2)||0, Number(inputs.x3)||0, Number(inputs.x4)||0];
      const ys = [Number(inputs.y1)||0, Number(inputs.y2)||0, Number(inputs.y3)||0, Number(inputs.y4)||0];
    ...
    workedExample: {
      ...
      inputs: { x1: 10, y1: 12, x2: 20, y2: 22, x3: 30, y3: 35, x4: 40, y4: 41 },
    ...
    faqs: [
      { question: 'Worin unterscheidet sich Kovarianz von Korrelation?', answer: 'Die Kovarianz ist maßstabsabhängig (hat die Einheit X mal Y). Der Korrelationskoeffizient ist dimensionslos und liegt stets zwischen -1 und +1.' },
    ],
  ...
  // 15. Variationskoeffizient Rechner
  {
    ...
    inputs: [
      { id: 'meanVal', label: 'Mittelwert (x̄)', type: 'number', defaultValue: 120, step: 0.1, min: 0.0001 },
      { id: 'sdVal', label: 'Standardabweichung (s)', type: 'number', defaultValue: 18, step: 0.1, min: 0 },
    ],
    calculate: `(inputs) => {
    ...
    faqs: [
      { question: 'Darf der Variationskoeffizient bei negativen Werten berechnet werden?', answer: 'Nein, der CV ist nur für verhältnisskalierte Größen mit echtem Nullpunkt sinnvoll definiert (z.B. Körpergröße, Preise, Messzeiten, aber nicht Celsius-Temperatur).' },
    ],
  ...
  // 16. Geometrisches Mittel Rechner
  {
    ...
    inputs: [
      { id: 'f1', label: 'Wachstumsfaktor Jahr 1 (z.B. 1.10 für +10 %)', type: 'number', defaultValue: 1.10, step: 0.01, min: 0.0001 },
      { id: 'f2', label: 'Wachstumsfaktor Jahr 2 (z.B. 1.25 für +25 %)', type: 'number', defaultValue: 1.25, step: 0.01, min: 0.0001 },
      { id: 'f3', label: 'Wachstumsfaktor Jahr 3 (z.B. 0.90 für -10 %)', type: 'number', defaultValue: 0.90, step: 0.01, min: 0.0001 },
      { id: 'f4', label: 'Wachstumsfaktor Jahr 4 (z.B. 1.15 für +15 %)', type: 'number', defaultValue: 1.15, step: 0.01, min: 0.0001 },
    ],
    calculate: `(inputs) => {
    ...
    workedExample: {
      ...
      inputs: { f1: 1.10, f2: 1.25, f3: 0.90, f4: 1.15 },
    ...
    faqs: [
      { question: 'Warum darf das geometrische Mittel nicht für negative Faktoren berechnet werden?', answer: 'Weil aus negativen Produkten keine reelle gerade Wurzel gezogen werden kann. Verwenden Sie daher immer Faktoren (1 + Rendite) statt reiner Prozentzahlen.' },
    ],
  ...
  // 17. Harmonisches Mittel Rechner
  {
    ...
    name: 'Harmonisches Mittel Rechner (Geschwindigkeit & Kurse)',
    ...
    inputs: [
      { id: 'valA', label: 'Geschwindigkeit Hinfahrt (km/h)', type: 'number', defaultValue: 60, step: 1, min: 1 },
      { id: 'valB', label: 'Geschwindigkeit Rückfahrt (km/h)', type: 'number', defaultValue: 120, step: 1, min: 1 },
    ],
    calculate: `(inputs) => {
    ...
    faqs: [
      { question: 'Wann muss das harmonische Mittel verwendet werden?', answer: 'Immer dann, wenn ein Durchschnitt aus Quotienten gebildet wird, deren Bezugsgröße im Nenner fixiert ist (z.B. Kilometer pro Stunde bei fester Wegstrecke).' },
    ],
  ...
  // 18. Bayes-Theorem Rechner
  {
    ...
    inputs: [
      { id: 'priorA', label: 'Basisrate / Prävalenz P(A) (%)', type: 'number', defaultValue: 1.0, step: 0.1, min: 0.001, max: 99.9 },
      { id: 'sensitivity', label: 'Sensitivität P(B|A) (%) – Richtig-Positiv', type: 'number', defaultValue: 95.0, step: 0.5, min: 0.1, max: 100 },
      { id: 'specificity', label: 'Spezifität P(¬B|¬A) (%) – Richtig-Negativ', type: 'number', defaultValue: 95.0, step: 0.5, min: 0.1, max: 100 },
    ],
    calculate: `(inputs) => {
    ...
    workedExample: {
      ...
      inputs: { priorA: 1.0, sensitivity: 95.0, specificity: 95.0 },
    ...
    faqs: [
      { question: 'Was ist der Unterschied zwischen Sensitivität und Spezifität?', answer: 'Sensitivität ist die Fähigkeit des Tests, Kranke zuverlässig als positiv zu erkennen. Spezifität ist die Fähigkeit, Gesunde zuverlässig als negativ zu klassifizieren.' },
    ],
    relatedSlugs: ['wahrscheinlichkeit-wuerfel-muenze-rechner', 'binomialverteilung-rechner', 'prozentrechner'],
  },

  // 19. Wahrscheinlichkeit Würfel & Münze Rechner
  {
    ...
    inputs: [
      { id: 'diceCount', label: 'Anzahl Würfel (W6)', type: 'number', defaultValue: 3, step: 1, min: 1, max: 10 },
      { id: 'targetRoll', label: 'Mindestens einmal Augenzahl 6 gewünscht', type: 'number', defaultValue: 1, step: 1, min: 1, max: 6 },
    ],
    calculate: `(inputs) => {
    ...
    workedExample: {
      ...
      inputs: { diceCount: 3, targetRoll: 1 },
      resultSummary: '42,13 %',
    },
    content: {
    ...
    faqs: [
      { question: 'Verdoppelt sich die Chance auf eine 6, wenn man zwei Würfel wirft?', answer: 'Nein! Bei zwei Würfeln beträgt die Chance 1 - (5/6)² = 30,56 % (nicht 2 × 16,67 % = 33,33 %), da beide Würfel gleichzeitig eine 6 zeigen können.' },
    ],
  ...
  // 20. Perzentil Rechner
  {
    ...
    inputs: [
      { id: 'pRank', label: 'Gesuchtes Perzentil P (z.B. 90 für 90. Perzentil)', type: 'number', defaultValue: 90, step: 1, min: 1, max: 99 },
      { id: 'd1', label: 'Messwert 1', type: 'number', defaultValue: 10, step: 0.1 },
      { id: 'd2', label: 'Messwert 2', type: 'number', defaultValue: 25, step: 0.1 },
      { id: 'd3', label: 'Messwert 3', type: 'number', defaultValue: 40, step: 0.1 },
      { id: 'd4', label: 'Messwert 4', type: 'number', defaultValue: 55, step: 0.1 },
      { id: 'd5', label: 'Messwert 5', type: 'number', defaultValue: 70, step: 0.1 },
      { id: 'd6', label: 'Messwert 6', type: 'number', defaultValue: 85, step: 0.1 },
    ],
    calculate: `(inputs) => {
    ...
    workedExample: {
      ...
      inputs: { pRank: 90, d1: 10, d2: 25, d3: 40, d4: 55, d5: 70, d6: 85 },
    ...
    faqs: [
      { question: 'Was bedeutet es, beim Gehalt im 90. Perzentil zu liegen?', answer: 'Es bedeutet, dass 90 % der Vergleichsgruppe weniger verdienen als man selbst und nur 10 % mehr.' },
    ],
    relatedSlugs: ['quartile-box-plot-rechner', 'mittelwert-median-modus-rechner', 'z-score-normalverteilung-rechner'],
  },

  // 21. Effektstärke Cohen's d Rechner
  {
    ...
    inputs: [
      { id: 'm1', label: 'Mittelwert Gruppe 1 (x̄1)', type: 'number', defaultValue: 105, step: 0.1 },
      { id: 'sd1', label: 'Standardabweichung Gruppe 1 (s1)', type: 'number', defaultValue: 15, step: 0.1, min: 0.001 },
      { id: 'm2', label: 'Mittelwert Gruppe 2 (x̄2)', type: 'number', defaultValue: 95, step: 0.1 },
      { id: 'sd2', label: 'Standardabweichung Gruppe 2 (s2)', type: 'number', defaultValue: 15, step: 0.1, min: 0.001 },
    ],
    calculate: `(inputs) => {
    ...
    workedExample: {
      ...
      inputs: { m1: 105, sd1: 15, m2: 95, sd2: 15 },
    ...
    faqs: [
      { question: "Was besagt die Faustregel von Cohen?", answer: "d = 0,2 gilt als kleiner Effekt, d = 0,5 als mittlerer Effekt und d ab 0,8 als großer Effekt." },
    ],
    relatedSlugs: ['t-test-rechner', 'p-wert-hypothesentest-rechner', 'standardabweichung-rechner'],
  },

  // 22. Chi-Quadrat Unabhängigkeitstest Rechner
  {
    ...
    inputs: [
      { id: 'cellA', label: 'Zelle A (Merkmal 1 Ja / Merkmal 2 Ja)', type: 'number', defaultValue: 30, step: 1, min: 0 },
      { id: 'cellB', label: 'Zelle B (Merkmal 1 Ja / Merkmal 2 Nein)', type: 'number', defaultValue: 20, step: 1, min: 0 },
      { id: 'cellC', label: 'Zelle C (Merkmal 1 Nein / Merkmal 2 Ja)', type: 'number', defaultValue: 15, step: 1, min: 0 },
      { id: 'cellD', label: 'Zelle D (Merkmal 1 Nein / Merkmal 2 Nein)', type: 'number', defaultValue: 35, step: 1, min: 0 },
    ],
    calculate: `(inputs) => {
    ...
    workedExample: {
      ...
      inputs: { cellA: 30, cellB: 20, cellC: 15, cellD: 35 },
    ...
    faqs: [
      { question: 'Was besagt der kritische Wert von 3,841?', answer: 'Bei einem Freiheitsgrad (df = 1) und einem Signifikanzniveau von α = 0,05 führt jeder χ²-Wert über 3,841 zur Ablehnung der Unabhängigkeitshypothese.' },
    ],
    relatedSlugs: ['p-wert-hypothesentest-rechner', 'bayes-theorem-rechner', 'korrelationskoeffizient-rechner'],
  },
];

const outputPath = path.join(__dirname, 'calcs-statistik.json');
fs.writeFileSync(outputPath, JSON.stringify(calculators, null, 2), 'utf8');
console.log('Saved calcs-statistik.json with ' + calculators.length + ' calculators');
