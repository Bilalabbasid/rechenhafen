...
for (const filePath of targetFiles) {
  ...
  for (const [slug, meta] of Object.entries(REGISTRY)) {
    // Check if file contains this slug
    const regex = new RegExp(`(slug:\\s*['"]${slug}['"][\\s\\S]*?relatedSlugs:\\s*\\[[^\\]]*\\])(,?)`, 'g');
    
    // Check if not already has isTimeSensitive
    const checkRegex = new RegExp(`slug:\\s*['"]${slug}['"][\\s\\S]*?isTimeSensitive:\\s*true`);
...
console.log(`Successfully added timeSensitiveMeta to ${updatedCount} calculators.`);
